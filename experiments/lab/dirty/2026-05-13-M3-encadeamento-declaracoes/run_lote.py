"""run_lote.py — M3 (encadeamento de declaracoes).

Diferente de M1/M2: aqui cada micro tem suas proprias pastas
output/, decoded/, debug/ AUTOCONTIDAS. A pasta resultados/
central contem apenas matriz consolidada.

Estrutura de saida:

  resultados/
    matriz_comparativa.md         comparacao entre micros
    matriz_bytes.csv              dados crus
    tokens/<dataset>.txt          tokens raiz (compartilhados)
  <micro>/
    output/<dataset>.tcf          encode
    decoded/<dataset>.csv         decode contra-prova
    debug/<dataset>.txt           detalhado

Para adicionar nova sintaxe:
  1. Criar pasta `<micro>/` com syntax.py e estrutura
     output/decoded/debug
...
def rodar_micro_em_dataset(syn, nome_ds, linhas, unicas, tokens,
    ...
    """Roda 1 micro em 1 dataset. Escreve output/, decoded/, debug/
    DENTRO do micro_dir."""
...
def gerar_relatorio(resultados, datasets, sintaxes_nomes, base_out):
    ...
    md = []
    md.append("# M3 — Matriz comparativa entre micros")
    md.append("")
