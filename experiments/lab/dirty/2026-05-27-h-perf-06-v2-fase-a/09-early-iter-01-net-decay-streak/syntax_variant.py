...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def __init__(self):
        self._trace = []
        self._rede = []
        # Variant H-PERF-06-v2 early-iter-01-net-decay-streak (opt-in).
        # Defaults OFF preserve byte-canonical D1-D9=1523B, D17a=322B.
        # Ative explicitamente atribuindo:
        #   inst.hcc_early_stop_streak = 3
        #   inst.hcc_early_stop_threshold = 0.05
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        # Variant H-PERF-06-v2 early-iter-01-net-decay-streak (opt-in).
        # Defaults OFF (None) preservam byte-canonical exato.
        _es_streak_limit = getattr(self, 'hcc_early_stop_streak', None)
        _es_threshold = getattr(self, 'hcc_early_stop_threshold', None)
        _es_enabled = bool(_es_streak_limit) and bool(_es_threshold)
        best_net_initial = None
        low_net_streak = 0
        ...
        while True:
            ...
            # Variant: opt-in early-stop por streak de net abaixo de threshold
            # relativo ao primeiro best_net. Quando _es_enabled = False
            # (defaults None) este bloco e' no-op total e o comportamento
            # canonical do M8.A e' preservado byte-a-byte.
            if _es_enabled:
                if best_net_initial is None:
                    best_net_initial = best_net
                elif best_net < _es_threshold * best_net_initial:
                    low_net_streak += 1
                    if low_net_streak >= _es_streak_limit:
                        iter_info['early_stopped'] = 'net_decay_streak'
                        iter_traces.append(iter_info)
                        break
                else:
                    low_net_streak = 0
