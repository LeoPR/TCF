...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            # H-PERF-06-v2 prune-k-03: adaptive min_k by iter.
            # K=2 picks need baseline > n_tam. K=2 max baseline for
            # typical small atoms is ~5-7 chars; once n_tam_est exceeds
            # threshold, K=2 is (mostly) unprofitable.
            n_tam_est = len(str(atom_count + comp_acc_k + 1))
            min_k = 2 if n_tam_est <= 2 else 3
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        ...
                        for a in range(len(refs)):
                            for b in range(a + min_k, len(refs) + 1):
                                sub = tuple(refs[a:b])
                                contagem[sub] += 1
