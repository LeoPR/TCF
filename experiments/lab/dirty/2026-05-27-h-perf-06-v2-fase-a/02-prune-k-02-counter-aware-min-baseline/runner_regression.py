"""H-PERF-06-v2 Fase A — regression runner for candidate
prune-k-02-counter-aware-min-baseline.

Monkey-patches `tcf.composicional.syntax.M8AVirtualRefsSyntax._detect_compositions`
with the variant version (hoisted line-292 width-guard for K=2 atomic subs;
mixed/virtual K=2 always included to preserve byte-canonical). Then runs
`from tcf import encode` on D1-D9 (single-col) + D17a (multi-col), comparing
bytes against the canonical M10 baseline (1523 / 322).

Per-iteration logic: variant skips pure-atomic K=2 sub-tuples whose
`wa+wb+1 <= n_tam_est` (those would be filtered at line 292 anyway). Mixed
K=2 with any virtual ref is forwarded to the existing filter, so the rationale
edge-case noted in the verdict is handled.
"""
...
spec = importlib.util.spec_from_file_location(
    "syntax_variant_prune_k_02", str(VARIANT_PATH)
)
...
def load_single_col(path: Path) -> list[str]:
    """D1-D9: single-column CSV with header (skipped)."""
...
def load_multi_col(path: Path) -> dict[str, list[str]]:
    """D17a: multi-column dict {col: [str, ...]}."""
    with path.open("r", encoding="utf-8", newline="") as fp:
        reader = csv.reader(fp)
        header = next(reader)
...
def main() -> int:
    print("[regression] variant: prune-k-02-counter-aware-min-baseline")
    ...
    d17a_bytes = len(d17a_out.encode("utf-8"))
    print(
