...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            # PRUNE-SINGLETON-01: two-pass discover/skip.
            # Pass A: count only (no sub_first_line populated).
            contagem = Counter()
            ...
            # Pass B: populate sub_first_line ONLY for non-singletons.
            sub_first_line = {}
            for li, pieces in enumerate(pieces_per_line):
                if pieces is None:
                    continue
                for p in pieces:
                    if p[0] == 'refs':
                        ...
                        for a in range(n_refs):
                            for b in range(a + 2, n_refs + 1):
                                sub = tuple(refs[a:b])
                                if contagem[sub] >= 2 and sub not in sub_first_line:
