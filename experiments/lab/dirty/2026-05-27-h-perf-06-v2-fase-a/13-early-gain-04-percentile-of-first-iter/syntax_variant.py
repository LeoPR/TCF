...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def __init__(self):
        self._trace = []
        self._rede = []
        # H-PERF-06-v2 Fase A, candidato early-gain-04-percentile-of-first-iter.
        # Opt-in: 0 = canonical (sem early-stop). Valores opt-in: 0.05, 0.01, 0.005.
        # Quando >0, para o loop em `_detect_compositions` quando
        # best_net < ratio * max_net_seen.
        self.early_gain_peak_ratio = 0
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        alias_to_sub = {}
        iter_traces = []
        # H-PERF-06-v2 candidato early-gain-04: track peak gain seen so far.
        # Quando self.early_gain_peak_ratio > 0, para o loop assim que best_net
        # cair abaixo de ratio * max_net_seen.
        max_net_seen = 0
        ...
        while True:
            ...
            # H-PERF-06-v2 early-gain-04: opt-in early termination.
            # Quando self.early_gain_peak_ratio > 0 e max_net_seen > 0 (i.e.
            # ja' temos um peak de referencia), para o loop quando o ganho do
            # candidato atual cair abaixo de ratio * max_net_seen. Default
            # ratio=0 -> guard curto-circuita e nada muda.
            if (best is not None
                    and max_net_seen > 0
                    and self.early_gain_peak_ratio > 0
                    and best_net < self.early_gain_peak_ratio * max_net_seen):
                iter_info['stopped'] = 'below_peak_ratio'
                iter_info['best_net'] = best_net
                iter_info['max_net_seen'] = max_net_seen
                iter_traces.append(iter_info)
                break
            if best is not None and best_net > max_net_seen:
                max_net_seen = best_net
