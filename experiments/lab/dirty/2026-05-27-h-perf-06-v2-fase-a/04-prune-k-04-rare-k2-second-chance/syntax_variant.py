...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        """Variant prune-k-04-rare-k2-second-chance.

        Two-pass com upper-bound prefilter pra K=2 enumeration:
          Pass A: enumera K>=3 only + cheap single-ref Counter.
                  Computa best_net_peek (best K>=3 net) e upper bound
                  conservativo em K=2 net.
          Pass B (condicional): re-enumera com K=2 SE upper-bound > best_net_peek
                  OU best_net_peek == 0. Quando roda, re-faz Counter
                  inteiro pra preservar ordem-de-insercao tie-break
                  byte-canonical.

        Idea: na maioria das iteracoes (cf. profile H-PERF-06) K>=3 domina
        e pass B nao roda → economiza o inner K=2 enumeration loop. Upper
        bound CONSERVADOR usa:
          R_max_k2 <= max(single_ref_count.values())
          baseline_max_k2 = 2 * len(str(max_atom_id)) + 1
          n_tam_k2 = len(str(atom_count + comp_acc_k + 1))
        Cada bound e' provadamente >= o real (single_ref count >= pair
        count; baseline_max usa o atom ID maximo possivel).

        Retorna (alias_to_sub, iter_traces). MESMA semantica do canonical
        em iter onde pass B roda (re-enumera tudo). Iter onde pass B
        skip: K=2 garantidamente nao podia ganhar.
        """
        next_alias = 1
        comp_acc_k = 0
        alias_to_sub = {}
        ...
        while True:
            # ---- Pass A: K >= 3 only + cheap single-ref stats ----
            contagem = Counter()
            sub_first_line = {}
            single_ref_count = Counter()
            for li, pieces in enumerate(pieces_per_line):
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        refs = p[1]
                        # Single-ref count (cheap, O(len(refs)))
                        for r in refs:
                            single_ref_count[r] += 1
                        # K >= 3 enumeration
                        for a in range(len(refs)):
                            for b in range(a + 3, len(refs) + 1):
                                sub = tuple(refs[a:b])
                                contagem[sub] += 1
                                if sub not in sub_first_line:
                                    sub_first_line[sub] = li

            # ---- Peek best K>=3 net (light scan) ----
            # Build provisional alias_first_line
            alias_first_line_peek = {}
            for li, pieces in enumerate(pieces_per_line):
                if pieces is None:
                    continue
                for p in pieces:
                    if p[0] == 'refs':
                        for ref in p[1]:
                            if ref < 0:
                                a = -ref
                                if a not in alias_first_line_peek:
                                    alias_first_line_peek[a] = li
            ...
            for sub, R in contagem.items():
                ...
                if virtual_count == 1:
                    ...
                    if virt_pos > 0:
                        ...
                        if alias_first_line_peek.get(virt_alias,
                ...
                if net > best_net_peek:
                    best_net_peek = net

            # ---- Upper bound K=2 net (CONSERVATIVE) ----
            R_max_k2 = max(single_ref_count.values(), default=0)
            max_atom_id_seen = atom_count + comp_acc_k
            if max_atom_id_seen <= 0:
                max_atom_id_seen = 1
            len_max_id = len(str(max_atom_id_seen))
            baseline_max_k2 = 2 * len_max_id + 1
            n_tam_k2 = len(str(atom_count + comp_acc_k + 1))
            net_upper_k2 = (R_max_k2 - 1) * max(0, baseline_max_k2 - n_tam_k2)

            need_k2_pass = (best_net_peek == 0) or (net_upper_k2 > best_net_peek)

            # ---- Pass B (conditional): re-enumera com K=2 included ----
            # Re-enumeramos do zero (Counter + sub_first_line) pra
            # preservar ordem-de-insercao tie-break byte-canonical.
            if need_k2_pass:
                contagem = Counter()
                sub_first_line = {}
                for li, pieces in enumerate(pieces_per_line):
