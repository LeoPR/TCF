...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def __init__(self):
        self._trace = []
        self._rede = []
        # H-PERF-06-v2 Fase A candidate early-gain-01:
        # Threshold relativo para parar HCC quando best_net cai abaixo de X%
        # do tamanho atual do body. Default 0 = comportamento canonical
        # (byte-canonical preservado D1-D9=1523B, D17a=322B).
        # Opt-in via valor positivo (0.05 = 5%).
        self.early_gain_threshold = 0.0
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            # H-PERF-06-v2 Fase A candidate early-gain-01-net-below-5pct-baseline:
            # early-termination by relative gain. Quando opt-in (threshold > 0),
            # estima cheaply o tamanho atual do body e para se best_net/body_size
            # cair abaixo do threshold (e.g., 0.05 = 5%). Default 0 = no-op,
            # preserva byte-canonical D1-D9 e D17a.
            if best is not None and self.early_gain_threshold > 0:
                # Estimate current body size cheaply: sum dos chars dos refs
                # (1 sep + len(str(abs(ref))) por elemento em cada 'refs' piece).
                body_size = 0
                for pieces in pieces_per_line:
                    ...
                    for p in pieces:
                        if p[0] == 'refs':
                            for r in p[1]:
                                body_size += len(str(abs(r))) + 1
                if body_size > 0 and best_net / body_size < self.early_gain_threshold:
                    iter_info['stopped'] = 'below_threshold'
                    iter_info['best_net'] = best_net
                    iter_info['body_size'] = body_size
                    iter_traces.append(iter_info)
                    break
