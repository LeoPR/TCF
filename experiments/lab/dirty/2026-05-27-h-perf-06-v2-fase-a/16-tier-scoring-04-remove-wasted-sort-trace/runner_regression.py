"""H-PERF-06-v2 Fase A — regression runner for tier-scoring-04-remove-wasted-sort-trace.

Monkey-patches M8AVirtualRefsSyntax._detect_compositions and _build_trace
with the versions from syntax_variant.py (sort deferred to _build_trace),
then runs `from tcf import encode` on D1-D9 + D17a, comparing bytes against
the canonical baselines (D1-D9 = 1523, D17a = 322).

Expected: byte-identical to canonical (sort was never read by selection logic).
"""
...
spec = importlib.util.spec_from_file_location(
    "syntax_variant_tier_scoring_04", str(VARIANT_PATH)
)
...
# Replace the methods on the canonical class. HCCSeqRLE inherits from
# M8AVirtualRefsSyntax and does NOT override _detect_compositions or
# _build_trace, so subclasses pick up the patch automatically.
...
def main() -> int:
    print("[regression] variant: tier-scoring-04-remove-wasted-sort-trace")
