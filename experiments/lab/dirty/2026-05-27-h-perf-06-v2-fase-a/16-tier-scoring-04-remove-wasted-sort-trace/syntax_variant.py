...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            # H-PERF-06-v2 tier-scoring-04: remove wasted sort.
            # Sort do `candidates` no iter loop e' PURO overhead — resultado so'
            # e' lido por _build_trace post-hoc. Defer sort para dentro de
            # _build_trace (computed on demand). 'n_pairs' tambem trocado
            # de generator-sum para len([... ]) — mesmo valor integer, evita
            # uma terceira passada por contagem.values().
            # Bytes inalterados: pick logic (linhas anteriores) usa Counter
            # insertion order, sort nao realimenta selection.
            iter_info = {
                'n_pairs': len([s for s in contagem if contagem[s] >= 2]),
                'n_candidates': len(candidates),
                '_candidates_raw': candidates,  # unsorted; lazy-sorted no _build_trace
                'picked': best,
    ...
    def _build_trace(self, iter_traces, prov_to_final,
        ...
        for info in iter_traces:
            ...
            if info['picked'] is None:
                ...
                t.append("")
                continue
            # H-PERF-06-v2 tier-scoring-04: lazy-sort here (on demand) instead
            # of eagerly sorting every iter no detector loop.
            if 'candidates_sorted' not in info:
                info['candidates_sorted'] = sorted(
                    info.get('_candidates_raw', []), reverse=True,
                    key=lambda c: c[0])
