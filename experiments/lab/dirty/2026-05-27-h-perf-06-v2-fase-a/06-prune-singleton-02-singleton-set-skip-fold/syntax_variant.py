...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            # Pass A: contagem only (unchanged from baseline iteration)
            contagem = Counter()
            ...
            # Materialize singleton set (one-shot)
            singletons = {s for s, r in contagem.items() if r == 1}

            # Pass B: filter sub_first_line population (skip singletons)
            sub_first_line = {}
            for li, pieces in enumerate(pieces_per_line):
                if pieces is None:
                    continue
