...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            #
            # H-PERF-06-v2 tier-scoring-01: two-pass com upper-bound prune.
            # Pass 1: iterar contagem (preserva Counter order), aplicar
            #   filtros baratos + UB-prune; nunca chamar _estimate_baseline_chars
            #   se sub provavelmente perde.
            # Pass 2: para survivors, computar baseline real e adicionar
            #   a candidates (mesma ordem de Pass 1, que e' Counter order).
            #
            # UB formula: baseline real <= K * n_est_max + (K-1) commas, onde
            #   n_est_max = max(2, len(str(atom_count + comp_acc_k + 1))) — IGUAL
            #   ao usado em _estimate_baseline_chars (linha 367), garantindo
            #   que UB e' matematicamente valido (atom run unica = max(2, ...)
            #   chars por digito; virtual = "9"*n_est = n_est chars; em ambos
            #   casos cada elemento contribui <= n_est_max chars).
            #
            # Nota: o sketch original do candidato sugeria `if net <= best_lb // 2: pass`
            # no Pass 2 mas o proprio autor admitiu confusao. Removido — qualquer prune
            # ativo no Pass 2 poderia mudar o picked candidate. Mantemos Pass 2 fiel.
            n_est_max = max(2, len(str(atom_count + comp_acc_k + 1)))

            # Pass 1: filtros baratos + UB-prune. Mantem ordem Counter.
            pending = []  # (sub, R, K, n_tam) em ordem Counter
            best_lb = 0
            for sub, R in contagem.items():
                ...
                # Upper bound: cada elemento <= n_est_max chars + K-1 separadores `,`.
                ub_baseline = K * n_est_max + (K - 1)
                if ub_baseline <= n_tam:
                    continue
                ub_net = (R - 1) * (ub_baseline - n_tam)
                if ub_net <= best_lb:
                    # SAFE prune: sub nao pode bater best_lb.
                    continue
                # Lower bound: cada elemento >= 1 char + K-1 separadores.
                lb_baseline = K + (K - 1)
                if lb_baseline > n_tam:
                    lb_net = (R - 1) * (lb_baseline - n_tam)
                    if lb_net > best_lb:
                        best_lb = lb_net
                pending.append((sub, R, K, n_tam))

            # Pass 2: baseline real apenas para survivors. Ordem = Counter order,
            # identica ao loop original (que tambem iterava contagem.items()),
            # garantindo tie-break preservado no pick loop adiante.
            candidates = []
            for sub, R, K, n_tam in pending:
