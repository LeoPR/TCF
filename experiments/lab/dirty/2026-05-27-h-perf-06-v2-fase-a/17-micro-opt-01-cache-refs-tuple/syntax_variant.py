    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            for li, pieces in enumerate(pieces_per_line):
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        # micro-opt-01: cache tuple(refs) once per piece;
                        # tuple slicing returns a tuple directly (1 alloc
                        # instead of list slice + tuple()). setdefault
                        # collapses the in-check + insert into a single
                        # hash lookup.
                        refs_t = tuple(p[1])
                        Lr = len(refs_t)
                        for a in range(Lr):
                            for b in range(a + 2, Lr + 1):
                                sub = refs_t[a:b]
                                contagem[sub] += 1
                                sub_first_line.setdefault(sub, li)
            ...
            for li in range(len(pieces_per_line)):
                ...
                for p in pieces:
                    if p[0] != 'refs':
                        novos.append(p)
                        continue
                    # micro-opt-01: cache tuple once; tuple slicing is a
                    # single allocation and comparison stays element-wise.
                    refs_t = tuple(p[1])
                    Lr = len(refs_t)
                    new_refs = []
                    i = 0
