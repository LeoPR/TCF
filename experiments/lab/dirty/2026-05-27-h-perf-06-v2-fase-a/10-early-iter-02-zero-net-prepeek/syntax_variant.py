...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            # === H-PERF-06-v2 Fase A: early-iter-02-zero-net-prepeek ===
            # Pre-peek: se max net entre candidates < MIN_USEFUL_NET,
            # para sem substituir. Variante mais agressiva (sem streak).
            # RISK byte-canonical: high — omite aliases net=1.
            MIN_USEFUL_NET = 2
            if candidates:
                max_possible_net = max(c[0] for c in candidates)
                if max_possible_net < MIN_USEFUL_NET:
                    iter_info = {
                        ...
                        'early_stopped': 'sub_epsilon_net',
                        'max_net_seen': max_possible_net,
                        'stopped': True,
                    }
