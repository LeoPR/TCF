"""run_lote.py — M11 (welding step 1: alg16 em src/tcf/core/).

Smoke test do welding step 1: M8.A composicional usa
`src/tcf/core/online.py` (copia canonica em src/) em vez do local
da macro. Validacao: bytes byte-a-byte identicos a M10/M9.

Esta macro confirma que a copia de alg16 para src/ esta funcional.
"""
...
# Welding step 1: alg16 (online.py) vem de src/tcf/core/.
# Ordem sys.path:
#   [0] = src/tcf/core (resolve `from online import ...`)
#   [1] = THIS (resolve `from syntax_base import ...`)
SRC_TCF_CORE = THIS.parents[3] / "src" / "tcf" / "core"
sys.path.insert(0, str(THIS))
sys.path.insert(0, str(SRC_TCF_CORE))
...
SINTAXES_REGISTRADAS = ["M8-A-baseline"]
...
def main():
    ...
    print(f"=== run_lote — M9 (stress adversarial) ===")
