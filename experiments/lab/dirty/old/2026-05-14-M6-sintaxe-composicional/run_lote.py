"""run_lote.py — M6 (sintaxe composicional)."""
...
def rodar_micro_em_dataset(syn, nome_ds, linhas, unicas, tokens,
    ...
    # Dump optimization trace if syntax provides it
    if hasattr(syn, 'get_trace'):
        trace_text = syn.get_trace()
        if trace_text:
            (debug_dir / f"{nome_ds}.optimization_trace.txt").write_text(
...
def gerar_relatorio(resultados, datasets, sintaxes_nomes, base_out):
    ...
    md = []
    md.append("# M6 — Matriz comparativa (sintaxe composicional)")
    md.append("")
...
def main():
    ...
    print(f"=== run_lote — M6 (sintaxe composicional) ===")
