"""H-PERF-06-v2 Fase A — Candidate 18 micro-opt-02-counter-to-dict-get
cProfile profile runner.

Espelha 16-tier-scoring-04/runner_profile.py — monkey-patch de
_detect_compositions aplicado ANTES do encode (igual ao runner_regression).

Steps:
1. Carrega syntax_variant.py via importlib.util.
2. Monkey-patches M8AVirtualRefsSyntax._detect_compositions (HCCSeqRLE
...
# Determinismo de hash (igual ao runner_regression).
...
spec = importlib.util.spec_from_file_location(
    "syntax_variant_18_micro_opt_02", str(VARIANT_PATH))
...
def main() -> int:
    ...
    print(f"[variant] patched _detect_compositions on M8AVirtualRefsSyntax + HCCSeqRLE")
