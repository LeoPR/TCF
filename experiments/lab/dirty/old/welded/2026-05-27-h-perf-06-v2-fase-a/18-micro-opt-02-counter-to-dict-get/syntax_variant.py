...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            # micro-opt-02: substitui Counter() por dict puro com bound
            # method get local (LOAD_FAST). Counter herda __setitem__ e
            # iter() de dict; insertion-order preservada desde 3.7 — igual
            # a Counter. Tie-break em pick (linha 300-302) e' byte-canonical.
            contagem = {}
            contagem_get = contagem.get
            sub_first_line = {}
                    ...
                    if p[0] == 'refs':
                        ...
                        for a in range(len(refs)):
                            for b in range(a + 2, len(refs) + 1):
                                sub = tuple(refs[a:b])
                                contagem[sub] = contagem_get(sub, 0) + 1
