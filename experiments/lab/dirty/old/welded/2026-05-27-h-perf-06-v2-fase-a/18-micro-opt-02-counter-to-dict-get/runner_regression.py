"""H-PERF-06-v2 Fase A — regression runner for micro-opt-02-counter-to-dict-get.

Monkey-patches M8AVirtualRefsSyntax._detect_compositions with the version
from syntax_variant.py (Counter() trocado por dict + get bound method local),
roda `from tcf import encode` em D1-D9 + D17a, e compara bytes contra os
baselines canonicos (D1-D9 = 1523, D17a = 322).

Expected: byte-identico ao canonical. Counter e dict tem insertion-order
identica desde 3.7, entao tie-break em pick e' preservado bit-a-bit.
"""
...
spec = importlib.util.spec_from_file_location(
    "syntax_variant_micro_opt_02", str(VARIANT_PATH)
)
...
# Replace the method on the canonical class. HCCSeqRLE inherits from
# M8AVirtualRefsSyntax and does NOT override _detect_compositions, so
...
def main() -> int:
    print("[regression] variant: micro-opt-02-counter-to-dict-get")
