...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            # - mantem ordem Counter (NAO ordena por R) para preservar tie-break
            #   e byte-canonical exato
            # - running-max inline (best_net atualizado a medida que itera)
            # - cheap upper-bound prune ANTES de chamar _estimate_baseline_chars
            #   evita o calculo mais caro para subs com R baixo + sub pequena
            #   que claramente nao podem bater best_net
            # - candidates_sorted ainda construido pois _build_trace consome
            #   (chamado incondicionalmente; gating quebra trace)
            #
            # Upper bound conservador para net = (R-1) * (baseline - n_tam):
            #   baseline <= K * n_est_ub + (K - 1)
            #   n_tam   >= len(str(atom_count + comp_acc_k + 1))  [K>=2 ja' garante]
            # onde n_est em _estimate_baseline_chars e':
            #   n_est = max(2, len(str(atom_count + comp_acc_k + 1)))
            # Usar n_est_ub = max(2, len(str(atom_count + comp_acc_k + len(contagem) + 9)))
            # garante margem extra contra qualquer borda (n_est_ub >= n_est).
            ...
            for sub, R in contagem.items():
                if R < 2:
                    continue
                K = len(sub)
                # Cheap upper-bound prune ANTES de virtual checks + baseline calc.
                # ub_net e' garantidamente >= net real (n_est_ub >= n_est,
                # n_tam_min <= n_tam para todo K >= 2).
                ...
                # PRESERVE Counter tie-break: only update if STRICTLY greater
                if net > best_net:
                    best_net = net
                    best = (sub, R)
