"""H-PERF-06-v2 Fase A - regression runner for micro-opt-01-cache-refs-tuple.

Monkey-patches M8AVirtualRefsSyntax._detect_compositions with the variant
version (which caches tuple(refs) per piece and uses setdefault instead of
in-check+insert in the inner O(R^2) loop, plus uses cached tuple in the
substitution loop), then runs `from tcf import encode` on D1-D9 + D17a,
comparing bytes against the canonical baselines (D1-D9 = 1523, D17a = 322).

Expected: byte-identical to canonical (pure idiom substitution, no
semantic change to Counter keys, dict semantics, or branching).
"""
...
# Replace _detect_compositions on the canonical class. HCCSeqRLE inherits
# from M8AVirtualRefsSyntax and does NOT override _detect_compositions, so
# subclasses pick up the patch automatically.
...
def main() -> int:
    print("[regression] variant: micro-opt-01-cache-refs-tuple")
