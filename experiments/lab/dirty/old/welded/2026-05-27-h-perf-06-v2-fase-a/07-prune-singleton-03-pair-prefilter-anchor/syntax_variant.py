...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            # VARIANT prune-singleton-03-pair-prefilter-anchor:
            # Pass A: count adjacent pairs only. Anti-monotonia (Apriori):
            # se par (a,b) e' singleton, qualquer sub-tupla contendo o par
            # adjacente tambem ocorre <=1 vez. Pulamos extensoes
            # garantidamente singletons antes de gera-las.
            count_pairs = Counter()
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        ...
                        for i in range(len(refs) - 1):
                            count_pairs[(refs[i], refs[i + 1])] += 1

            # Pass B: expand sub apenas onde todos os pares internos
            # sao non-singleton (count_pairs >= 2).
            contagem = Counter()
            sub_first_line = {}
            for li, pieces in enumerate(pieces_per_line):
                if pieces is None:
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        refs = p[1]
                        n_refs = len(refs)
                        for a in range(n_refs - 1):
                            # start at K=2 (pair a..a+1)
                            if count_pairs[(refs[a], refs[a + 1])] < 2:
                                continue  # toda extensao a partir daqui e' singleton
                            for b in range(a + 2, n_refs + 1):
                                # extensao valida apenas se ultimo par non-singleton
                                if b > a + 2 and count_pairs[(refs[b - 2], refs[b - 1])] < 2:
                                    break
                                sub = tuple(refs[a:b])
                                contagem[sub] += 1
