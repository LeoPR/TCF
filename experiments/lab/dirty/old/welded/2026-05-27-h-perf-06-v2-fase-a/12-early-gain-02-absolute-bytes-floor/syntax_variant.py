...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def __init__(self):
        self._trace = []
        self._rede = []
        # early-gain-02-absolute-bytes-floor (H-PERF-06-v2 Fase A, opt-in).
        # Default = 0 -> canonico (best_net comeca em 0 e so' aumenta; nunca
        # dispara). >0 = opt-in experimental: aborta loop quando proxima
        # substituicao economiza menos bytes que threshold. Quebra byte-canonical.
        self.early_gain_min_bytes = 0
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            # early-gain-02-absolute-bytes-floor: aborta loop quando best_net
            # cai abaixo de threshold absoluto em bytes. Default 0 = OFF.
            # Aplica ANTES de 'if best is None' pra preservar semantica do
            # caminho canonico (default 0 e' inerte: best_net >= 0 sempre).
            if (best is not None
                    and best_net < self.early_gain_min_bytes):
                iter_info['stopped'] = 'below_abs_threshold'
                iter_info['best_net'] = best_net
                iter_info['threshold'] = self.early_gain_min_bytes
