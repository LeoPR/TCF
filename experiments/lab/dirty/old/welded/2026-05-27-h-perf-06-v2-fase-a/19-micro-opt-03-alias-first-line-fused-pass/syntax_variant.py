...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            sub_first_line = {}
            alias_first_line = {}
            sub_setdefault = sub_first_line.setdefault
            alias_setdefault = alias_first_line.setdefault
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        ...
                        for a in range(Lr):
                            ref_a = refs[a]
                            if ref_a < 0:
                                alias_setdefault(-ref_a, li)
                            for b in range(a + 2, Lr + 1):
                                sub = tuple(refs[a:b])
                                contagem[sub] = contagem.get(sub, 0) + 1
                                sub_setdefault(sub, li)
