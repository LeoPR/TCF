...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        # H-PERF-06-v2 Fase A — Candidate prune-singleton-04
        # Sketch-based prefilter (Count-Min sketch simplificado, 64k slots).
        # Pass A: incrementa sketch[hash(sub) & MASK]; rastreia hashes >=2.
        # Pass B: so' insere no Counter se hash(sub) in maybe_freq.
        # Monotonia garante zero false negatives (subs com R>=2 sempre passam).
        # False positives sao filtrados pelo `if R < 2: continue` adiante.
        SKETCH_BITS = 16
        MASK = (1 << SKETCH_BITS) - 1

        while True:
            # ---- Pass A: sketch-only enumeration ----
            sketch = bytearray(1 << SKETCH_BITS)  # saturating at 2
            maybe_freq = set()  # hashes seen >=2
                    ...
                    if p[0] == 'refs':
                        ...
                        for a in range(n):
                            for b in range(a + 2, n + 1):
                                h = hash(tuple(refs[a:b])) & MASK
                                if sketch[h] < 2:
                                    sketch[h] += 1
                                    if sketch[h] == 2:
                                        maybe_freq.add(h)

            # ---- Pass B: real Counter, gated by sketch ----
            contagem = Counter()
            sub_first_line = {}
            for li, pieces in enumerate(pieces_per_line):
                if pieces is None:
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        ...
                        for a in range(n):
                            for b in range(a + 2, n + 1):
                                sub = tuple(refs[a:b])
                                if (hash(sub) & MASK) not in maybe_freq:
                                    continue  # provably singleton (mod hash collision)
                                contagem[sub] += 1
