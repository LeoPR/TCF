...
Antes do encode, aplica monkey-patch igual ao runner_regression.py:
  - substitui M8AVirtualRefsSyntax._detect_compositions pelo do
    syntax_variant.py (two-pass discover/skip singleton-prune)
...
def load_variant_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "syntax_variant_prune_singleton_01", str(variant_path)
    )
    mod = importlib.util.module_from_spec(spec)
    # Pre-imports defensivos (igual runner_regression.py)
    import tcf.core.online  # noqa: F401
    import tcf.core.syntax_base  # noqa: F401
...
def main() -> int:
    print("[variant-profile] aplicando monkey-patch (two-pass discover/skip)")
    apply_monkey_patch()

    # IMPORTANTE: importar `encode` apenas APOS o patch.
