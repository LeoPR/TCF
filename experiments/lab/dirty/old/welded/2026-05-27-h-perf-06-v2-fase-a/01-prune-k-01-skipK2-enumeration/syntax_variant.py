...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def __init__(self, min_k=2):
        self._trace = []
        self._rede = []
        # H-PERF-06-v2 Fase A prune-k-01: floor de K na enumeracao do
        # detector. min_k=2 = canonical (byte-identico). min_k=3 = prune.
        self._min_k = min_k
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            ...
            for li, pieces in enumerate(pieces_per_line):
                if pieces is None:
                    continue
                MIN_K = self._min_k  # H-PERF-06-v2 prune-k-01 gate
                for p in pieces:
                    if p[0] == 'refs':
                        refs = p[1]
