...
class M8AVirtualRefsSyntax(Syntax):
    ...
    def _detect_compositions(self, pieces_per_line, atom_count):
        ...
        while True:
            contagem = Counter()
            sub_first_line = {}
            # H-PERF-06-v2 Fase A: prune-k-02-counter-aware-min-baseline
            # Hoisted check: skip pure-atomic K=2 sub-tuples whose baseline
            # cannot exceed n_tam_est. Equivalent to the line-292 filter
            # for all-atomic K=2 because _estimate_baseline_chars(K=2 atomic)
            # returns exactly wa + 1 + wb. For mixed/virtual K=2 we ALWAYS
            # include (the existing _estimate_baseline_chars treats virtuals
            # as n_est-wide and we must not skip them prematurely — see
            # verdict rationale).
            n_tam_est = len(str(atom_count + comp_acc_k + 1))
            for li, pieces in enumerate(pieces_per_line):
                ...
                for p in pieces:
                    if p[0] == 'refs':
                        ...
                        Lr = len(refs)
                        for a in range(Lr):
                            # K=2 branch with hoisted width guard
                            if a + 1 < Lr:
                                ra = refs[a]
                                rb = refs[a + 1]
                                if ra < 0 or rb < 0:
                                    # Virtual present: defer to existing
                                    # _estimate_baseline_chars (always include).
                                    include_k2 = True
                                else:
                                    wa = len(str(ra)) if ra != 0 else 1
                                    wb = len(str(rb)) if rb != 0 else 1
                                    include_k2 = (wa + wb + 1 > n_tam_est)
                                if include_k2:
                                    sub = (ra, rb)
                                    contagem[sub] += 1
