...
| **H-GDICT-01 / cross-dict B2** | ❌ **GATE N≥5 FALHOU** (1/5 ≥15%) | [gate-result](../2026-06-27-gdict-b2-prototype/gate-result.md): só SNAP paga (−19.3%, caso afortunado); cit-HepTh **+30.6%** e email-Enron **+18.6%** o B2 PERDE (união cruza bucket → greedy recusa). Padrão anti-incidente 05-21. SEGURO (greedy self-gated, nunca regride) mas nicho estreito → NÃO weldar geral; `closed-insufficient-generalization` OU opt-in de nicho |
| **H-DICT-HIGHCARD** | 🔬 CARACTERIZADA → reduz a "descapar" (safe, pequeno) | [result](../2026-07-01-dict-highcard/result.md): NÃO é robusto (−16..−36% do B2 era PARCIAL/pair). Data-dependente: dict vence high-card **sem estrutura** (l_partkey −46%, municipio −40%, razao_social −40%); OBAT/HCC vence **com estrutura** (l_orderkey, ps_partkey ordenado, from_node agrupado). **N/K não prediz.** Conserto SEGURO: descapar `_V2B_MAX_CARD` → dict vira candidato do `min()` (byte-safe, min nunca regride; core.py:185-187); custo = compute. Weld sob aprovação (compute + re-pin) |
