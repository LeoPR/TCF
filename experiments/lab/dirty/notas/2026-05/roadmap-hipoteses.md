...
| | **PRIMEIRA MEDICAO 2026-08-14** (o owner reabriu com a grafia `0.3(14x0)4` / linha-fantasma). **O nucleo captura ZERO run intra-valor, e e' estrutural**: o OBAT so' compara EXTREMIDADES (`core/online.py:58-71`, sem offset interno em lugar nenhum) e o miolo vai VERBATIM pro wire (`core/online.py:234-236`); o HCC opera sobre ids de atomo, nao chars. Curva medida com ajuste EXATO: 1 valor `bytes = n+11`, 20 distintos `bytes = 20n+69` -> **1,000 B por char repetido por linha, zero amortizacao** ate' n=128. O OBAT tambem NAO dedupa run infixo entre valores distintos. **TETO no corpus: 0,0031%** (11 de 186 colunas tem run>=6, e elas se partem em 2 familias): padding de ID (`Clerk#000000004`, 99% dos casos) o OBAT+HCC **ja' comem como prefixo compartilhado** — e **colapsar os runs ali CUSTA 1.744 B (-2,31%)** em `o_clerk`, contra-prova medida; cauda de float (`wine.alcohol`, UNICA instancia) e' onde pagaria: **-2,47% na coluna / -45,1% nos 40 sujos**. Break-even em L~6-7. **BLOQUEIOS DA GRAFIA INLINE**: o `*` ja' e' o separador `lit->lit` (`syntax.py:607-617`, byte-load-bearing) — `0.3*14*0*4` colide com o parser; e o contador e' digito, que escapa (e' o proprio H-INTRA-02). Desbloqueio = **H-REF-03** (alfabeto por complemento), nao grafia nova. **E os mesmos 40 valores ja' sao atacados pela H-FLOAT-GRAFIA-01**: sao `n/30`, entao o run e' o SINTOMA e a fracao e' a CAUSA. Continua ADIADO, agora com numero. | aberta (medida, adiada) | nota [`2026-08-14-2010`](../2026-08/2026-08-14-2010-rle-intra-valor-e-perda-estatistica.md) |
