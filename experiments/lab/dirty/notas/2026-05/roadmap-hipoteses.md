...
| H-15-07 | **BUG PRE-EXISTENTE, achado pela revisao do ADR-0044**: as regex dos specs terminam em `$`, que em Python casa TAMBEM antes de um LF final. Um valor valido terminado em LF classifica como `compressible`, o LF e' descartado pelo filtro de simbolos, e o **RT PERDE o caractere**. RAIO MEDIDO 2026-08-21: atinge **CPF** (todos), **CNPJ** (todos, numerico e alfa) e **IP na forma NAO-padded** (`192.168.0.1` perde; `192.168.001.001` escapa porque cai antes em `format_padded_zeros`). **data-iso NAO e' atingido** — o classify dele CHECA O COMPRIMENTO (`len(v) != 10`), que e' justamente a defesa que os outros nao tem. NAO veio deste arco. | aberta — **NAO alcancavel pela API publica principal** (`tcf.encode` recusa LF fail-loud ANTES da nature), mas alcancavel por `tcf.natures.encode_value`, que esta' no `__all__` e no docstring do modulo. CONSERTO MEDIDO: `$` -> `\Z` em `_CPF_RE`/`_CNPJ_RE`/`_IPV4_RE` resolve 6/6 dos casos que perdiam, e custa **0 divergencia de byte** em 9.012 valores sem LF (`\Z` e `$` sao identicos quando nao ha LF no fim). **Toca 3 specs = precisa de aprovacao** | verificado 2026-08-21; classe do `BUG-CHAVE-VAZIA-POSICIONAL` (o formato altera o dado) |
