...
| H-15-08 | Assimetria do LF final entre rotas — e o LF como candidato a DROP na transmissao | **PRECIFICADO 2026-08-21 (ADR-0045 §3): nao uniformizar.** REFINADO no lab `0500`: o LF final **NAO e' load-bearing** — e' 100% recuperavel (drop+readd devolve o original em 55/55). O que trava o drop e' a ASSIMETRIA: o magic NAO determina a convencao (`#TCF.8M` e `#TCF.8b` emitem em uns casos e nao em outros), entao o receptor nao sabe recolocar — e dropar SEM recolocar perde valor vazio final EM SILENCIO. Excecao real: no `.8H` o LF esta' DENTRO do `size`, e ali e' byte contado, nao decoracao. Valor do drop: **4-6% em payload minusculo** (alvo declarado do `.8`, O-FMT-15/16). CUSTO ESCONDIDO: o warning de 'wire possivelmente truncado' e' um DETECTOR DE TRUNCAMENTO; dropar sistematicamente o vira ruido. | aberta como **candidato a modo de TRANSPORTE** (direcao `contrato-externalizado + aceleradores`), nao como mudanca de formato. So' viavel se as duas pontas concordarem numa regra POR ROTA | lab `2026-08-21-0500-lf-final-tem-funcao` |
