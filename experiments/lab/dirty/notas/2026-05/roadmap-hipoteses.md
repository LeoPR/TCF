...
Apos revisao conceitual (ver `2026-05-21-0138-revisao-conceitual.md`),
...
Tabela resumida em `2026-05-21-0138-revisao-conceitual.md` (categoria
...
[`2026-05-21-0138-revisao-conceitual.md`](2026-05-21-0138-revisao-conceitual.md)
...
| H-DA-02 | Pre-stage pode gerar dica generica (sem nomear tipo) que calibra OBAT | **subsumida** em H-DA-07 (nao testada isoladamente) | n/a | Sub-hipotese de H-DA-07, nao validada como pergunta separada |
| H-DA-03 | OBAT pode fazer comparacao relativa (`+N` abstrato) sem nomear unidade | **subsumida** em H-DA-07 (nao testada isoladamente) | n/a | Idem |
...
| H-DA-05 | Linhas pos-transicao podem formar novo run se OBAT mantiver alinhamento | **absorvida** em H-DA-07 | confirmada via H-DA-07 | n/a |
...
| H-PERF-05d | Counter incremental (re-conta so' partes afetadas) | **MEDIDO e FECHADO (measured-not-worth-weld)** 2026-06-24: incremental real ~1,2-1,7x pure-Python (NAO 4-5x, rebuild e' ~46% do encode, nao 92%), div +0,03-0,05% so' datetime, RT 100%. A "outra metade" (loop de candidatos) e' ~99% cheap-skip, cortavel so' com reescrita incremental substancial; ganho so' pure-Python (Cython ja' cobre ~2,67x). | `2026-06-24-h-perf-05d-recaracterizacao/` (re-carac) + `2026-05-22-...` (original) | Owner: retornos decrescentes -> nao vale tocar o core. Frente certa = port Cython, nao reescrita do algoritmo. |
...
| H-PERF-06 | Cython/Rust port de `lcp_len`/`lcs_len` corta Python overhead | **REFRAMED → WELDED** (2026-07-01, DB-7): lcp/lcs descartado; o alvo virou o HCC `_detect_compositions` → **prune welded ADR-0019** + **Cython welded ADR-0020** (~2.67x). Gate compilado = T-CI-3. | ADR-0019/0020 + `src/tcf/_core/detect.pyx` | (era "aberta/lcp-lcs"; ver linha "H-PERF-06 REFRAMED" no fim) |
...
| H-FIX-02 | Mesma opcao mas re-baseline M9 necessario (D1-D9 tem `,`) | **N/A** (Op A nao escolhida) | n/a | n/a |
...
| H-ED-04 | Header da coluna (ex: "tipo=numerico") permite supressao adicional | **adiada** (nao testada, dependeria de H-ED-01..03 funcionar) | n/a |
...
|---|---|---|---|
| H-DT-01 | Timestamps merecem encoder proprio. **GENERALIZADO -> H-STRUCT-01.** | subsumida por H-STRUCT-01 | n/a |
...
| H-IMPLICIT-SINGLECOL-01 | **Coluna unica TIPADA sem maquinaria hierarquica** (owner 2026-07-23): numa lista de escalares, `list`-ness/`count`/estrutura sao DEDUTIVEIS (linhas=elementos) -> implicitos; so' o TIPO e' irredutivel. Hoje `[1,2,3]`->`.8H` (envelope `#V`+`\z`+`#count`+`[]`) so' pra preservar tipo. Alvo: single-col tipado = body do orfao + marcador minimo (`#TCF.8:n`, desvio opt-in ADR-0030). String ja' e' 0-overhead (default implicito). | **baseline MEDIDA 2026-07-23**: overhead FIXO ~19-28 B no `.8H` (domina coluna pequena: 46-74%; some na grande). Equivalencia JSON 14/14. Refina o Passo 2. Sem codigo de core ainda. | nota [`2026-07-23-0259`](../2026-07/2026-07-23-0259-implicitude-singlecol-logica.md) + lab `2026-07-23-0300-implicitude-singlecol-massa/` |
| H-DENSE-MODE-01 | **Modo denso c/ marcador de binarizacao** (owner 2026-07-23): 2a camada ORTOGONAL a implicitude do tipo. Um marcador lider declara O QUE/COMO foi binarizado (tipo, bit-width, alfabeto) -> mesmo mecanismo serve bool (1 bit), low-card (bN), e campo binario arbitrario. Forma default = **base64** (textual/editor-safe + ganha pos-gzip); cru so' side-channel; hex fallback. Candidato de `min()` por-coluna (nunca-pior), NAO default incondicional. | **escada MEDIDA 2026-07-23**: base64 = +33% vs cru (84 vs 63 B) mas TEXTUAL; pos-gzip empata (37=37). Ganho so' na ALTA entropia (rand-50 1533->97); baixa entropia o `.8H` atual ja' vence (all-true 36<97). RT+equiv-JSON 35/35. ESTUDO ABERTO, sem weld. | nota [`2026-07-23-0345`](../2026-07/2026-07-23-0345-modo-denso-marcador-binarizacao.md) + lab `2026-07-23-0336-implicitude-bool-escada-base64/` |
...
| H-DA-12 | **Delta-of-delta (2a diferenca) para date**: `[1o ordinal, 1o delta, dd1, dd2, ...]`. E' o candidato classico de serie temporal (a ideia do Gorilla sobre timestamps) e era **o unico que o projeto NUNCA tinha medido**: o registry cobria delta de coluna (lab `0042`) e delta ciclico (ADR-0040), nunca a 2a diferenca. | **MEDIDA 2026-08-15** (lab `2026-08-15-0400-date-processo-de-compressao`, 6 transformacoes x 14 regimes, formato FIXO, **0 falhas**, cada uma validada pela propria inversa): **TEM NICHO PROPRIO**. Em `esparsa-ordenada` (saltos de 1..40 dias, crescente): nucleo 6637, ordinal 3854, delta 772, **delta2 605 B**, vence os tres, e e' **84,3% abaixo do ordinal welded**. Mecanismo: com saltos irregulares MAS crescentes, a 1a diferenca continua irregular (1..40) enquanto a **2a oscila em torno de ZERO**, alfabeto pequeno perto de zero e' o que o nucleo come bem. E' exatamente o caso que o `T-DATA-ALVO-DELTA` registrava como sobra (*"sobram pro delta-coluna: espalhado-ordenado (644)"*), **o delta2 faz 605**. Fora desse regime ele perde (diaria 25 vs 22 do ordinal; uteis 336 vs 30), entao e' candidato de `min()`, nunca default. | aberta (medida, nicho identificado) | lab `experiments/lab/dirty/2026-08/2026-08-15/2026-08-15-0400-date-processo-de-compressao` |
| H-DENSE-MODE-03 | **Largura do ESPACO DO TIPO em vez da cardinalidade do dominio**: o dominio NAO viaja (owner 2026-08-14, sobre a hora: *"talvez se ele tiver uma forma binarizada propria para o espaco de numeros dele, ja' que vai de 0-24 e 0-60"*). E' a **inversao consciente do ADR-0036** (*"densidade por CARDINALIDADE, nao por tipo declarado"*). **PRIOR ART**: o PRINCIPIO existe e esta' soldado, **ADR-0037** se chama literalmente *dominio IMPLICITO* (*"a pergunta nao e' como declarar o dominio, e' QUANDO ele pode NAO viajar: quando e' fixo por tipo"*), medido la': os **15 B** entre `b2` (79) e o bN tipado (94) *sao exatamente o dominio que deixou de viajar*. Mas so' p/ dominio FECHADO de 2-3 valores. **Derivar `w` de uma FAIXA (`0..23`->5 bits, `0..86399`->17): NAO EXISTE**, as 9 ocorrencias de `log2` no repo sao todas `ceil(log2 k)` sobre distintos observados. A H-DENSE-MODE-01 fala em bit-width e *campo binario arbitrario*, mas ali isso e' **blob de bytes**. E a HORA e' **o exemplo canonico do ponto cego**, registrado em **ADR-0018 desde 2026-05-27**: *"beijing `hour`, 24 unicos -> 228,8% de inflacao… entropia ~5 bits/linha, poderia ir pra <15%"*, nunca perseguido por esta via. | **MEDIDA 2026-08-14** (lab `2026-08-14-2320-hora-binarizada-pelo-tipo`, 6 casos x 5 formas + virada em 11 cardinalidades, **0 falhas**). **(1) POR CAMPO E POR ORDINAL CUSTAM O MESMO**: `5+6+6 = 17` bits e `ceil(log2 86400) = 17`, porque `2^5·2^6·2^6 = 131072 = 2^17`; medido **5682 vs 5683 B**. A escolha entre elas nao e' de tamanho. **(2) BINARIZAR DESTROI A ESTRUTURA**: nos regimes regulares o **ordinal DECIMAL vence por ordens de grandeza** (cada-minuto **53 B** vs 4095 do bit-packed = **77x**; cada-segundo **50 B** vs 5683 = **114x**), porque o decimal e' progressao aritmetica e o seq-RLE a esmaga *independente de n*, enquanto o bit-pack tem piso `n x 2,83 B` que **nada faz descer**. E' a mesma razao por que o `data-iso` escolheu decimal: *deixar a aritmetica visivel*. **(3) O NICHO E' PRECISO, e e' um limite de NAMESPACE**: a virada (com `k` isolado da regularidade por LCG) cai **exatamente onde o nucleo para de usar o bN**, header vai de `B87d0` (k=144, 3921 B) para `#TCF.8` literal (k=288, 9838 B contra **5683** do tipo). Motivo no codigo: `dominio_bn.py:70` **`MAX_W = 8`**, *teto do NAMESPACE: `w` de 1 a 8*, porque a largura ocupa **um digito so'** no header. **Acima de 256 distintos nao ha' candidato denso nenhum**, e' o buraco onde esta hipotese existiria. Algebra confere: denso vence enquanto `n < 6[k(L+1)+2]/(17-w)`, com `L=11` (grafia ESCAPADA `\08:\30:\00`); p/ `k=256,w=8` da' `n<2049`, e a medicao a n=2000 bate. **(4) NA COLUNA REAL** (`retail`, k=564, n=2000): texto 10619, ordinal-decimal 10120, **tipo-5+6+6 = 5682 (-46,5%)**. **3 ACHADOS DA ALGEBRA**: `w` e' **escolha de RESOLUCAO** (`HH:MM`->11 bits, `HH:MM:SS`->17, com ms->27; cada bit custa `n/6` B); **o null sai de GRACA** (`2^17=131072 > 86400` deixa **44.672 codigos livres**, como o slot 3 do `b2`; no bN o null pode empurrar `w`); e **o denso nao tem o teto de 256**. **BLOQUEIOS**: (a) **`w=17` nao cabe na gramatica**, o `<modo>` do indice 7 e' MONO-CHAR (`decoder.py:429`, `resto[:1]`), e essa e' a UNICA mudanca de wire exigida (o namespace de modo ja' reserva letras p/ *subtipos mapeados*, estado *PREPARADO, nao construido*); (b) **`pack_w` NAO valida faixa**, `format(3,'01b')` da' `'11'` sem truncar, entao `pack_w([3,0],1)` corrompe calado; hoje inalcancavel, mas um modo por espaco do tipo poe a checagem `0 <= v < 86400` na conta do chamador; (c) **`24:00:00` fica FORA de `0..86399`**, e' a peculiaridade (4) do fechamento da hora cobrando; (d) o ganho e' **TERMINAL** (base64 e cru empatam pos-gzip: 37 e 37 B; e o gate do bN deu 8,8% terminal vs **1,7% pos-brotli**). **3 CONDICOES DE APLICABILIDADE, medidas**: k > 256 **E** ordem irregular **E** leitura terminal. A coluna real satisfaz as tres. **LACUNA**: o dataset canonico da hipotese (`beijing-pm25.db`, o das 228,8% do ADR-0018) esta' com **0 bytes**. | lab `experiments/lab/dirty/2026-08/2026-08-14/2026-08-14-2320-hora-binarizada-pelo-tipo` |

| H-FLOAT-GRAFIA-01 | **Grafia fracional p/ dizimas** (owner 2026-08-14): `0.333333333333` -> `1/3…12` (fracao + nº de casas), **LOSSLESS por grafia**, so' muda o corpo, RT byte-a-byte; candidato do `min()` como a escala, SEM gate do Pacote 10. Achavel por fracao continua (`Fraction.limit_denominator`) + verificacao de re-emissao. Dizima genuina nasce de DIVISAO (rateio/parcelamento, o proprio exemplo do owner). Testar JUNTO com escala-com-excecoes (desenho ALP): provavel nicho, a escala deve dominar na frequencia. | **MEDIDA 2026-08-14** (lab `2026-08-14-1745-grafia-fracional-e-escala-com-excecoes`): mecanismo PROVADO (126/126 byte-a-byte; a verificacao por re-emissao AUTO-PROTEGE, recusa `2.718281828`/`0.30000000000000004`/`12.3456789` sozinha), ganho MEDIDO onde ha' dizimas DISTINTAS (`dizima-variada` 160->94 B, **-41%**) e quase nulo onde o RLE ja' resolvia (`dizima-uniforme` -3 B, o par de contra-prova isolou a causa). **MAS o corpus tem n=1**: `wine.alcohol` e' a unica coluna com dizima em 186 colunas / 31 float; 9 conversoes em 2000 la', ZERO nas outras 4 reais. Decidir por ela pede corpus com rateio/divisao (a propria origem que o owner descreveu: parcelamento). **Irma nova: escala-com-excecoes** (desenho ALP), resolve os DOIS modos de falha da escala pura e **reusa o `MARKER_LITERAL='_'` que o core ja' tem soldado**, nao precisa de grafia nova | nota [`2026-08-14-1739`](../2026-08/2026-08-14-1739-loss-e-lossless-alterado-pesquisa.md) + lab `2026-08-14-1745` |
...
| ID | Hipotese | Status | ref |
|---|---|---|---|
| H-LOSS-00 | **Meta-camada de contrato** (ABS/REL/DECIMALS/AGG/DIST) + marcador inspecionavel no header + teste valida-invariante. PRE-REQUISITO de toda perda. **VOCABULARIO DERIVADO 2026-08-14** (direcao do owner: *"orientado a estatistica de perdas e erros… se a perda significa algo como 1% numa soma ou multiplicacao"*), testado por reducao mutua contra 5 areas: **4 eixos irredutiveis + 1 qualificador**. `quantum` (grade absoluta, **estritamente mais forte que `abs`**: a norma monetaria exige o valor EXPRESSAVEL em centavos, nao apenas perto de um); `abs` (compoe sob SOMA); `rel` (compoe sob PRODUTO, GUM Eq.12; as duas operacoes que o owner citou pedem os DOIS eixos); `agg-exact` (restricao de CONJUNTO, nenhuma tolerancia pontual a implica; obriga ALOCACAO, nao arredondamento local); e **`mode`** (half-even/half-up/down/stochastic) como qualificador OBRIGATORIO, porque duas normas com o MESMO quantum divergem no modo e o modo decide o VIES, e' a distincao do HMRC (arredondar p/ baixo e' concedido a invoice traders porque *o erro cancela na contraparte*, e NEGADO a retalhistas porque sem contraparte o vies vira perda de receita). **`significativos` COLAPSA em `rel`** (s sig ~ eps 5e-s). Isto SUBSTITUI o "comecar minimal: DECIMALS + AGG-soma" da taxonomia §4, certo na direcao, curto em 2 eixos. **+2 achados fora dos eixos**: (i) **apresentacao != armazenamento**, HMRC manda calcular a 5-6 casas e apresentar a 2, e a GUM 7.2.6 manda reter guard digits; duas areas chegaram na mesma estrutura de 2 niveis; (ii) **coluna DERIVADA por subtracao nao e' coberta por vocabulario pontual nenhum**, pelo lema de Sterbenz a subtracao e' EXATA e o erro vem todo dos inputs ja' arredondados, com amplificacao |x|/|x-y| ilimitada; logo *gravar a diferenca vs deduzi-la do par* vira CLAUSULA DO CONTRATO DE ERRO, nao questao de bytes (toca materializacao-minimal e o DERIVED-DROP H-LOSS-02). **PROTOTIPADO 2026-08-14** (lab `2026-08-14-2110-parametro-de-tolerancia-float`, 12 pedidos x 3 colunas, **0 falhas**): a forma do parametro esta' DE PE', 4 eixos que compoem por AND + `mode`, tres estagios **derivar -> aplicar -> VERIFICAR**, e fail-loud quando a tolerancia nao e' realizavel. **PRIOR ART ACHADO**: `H-smart-rounding` (`docs/workbench/_archive/tickets/frozen/`, 2026-04-10, status OPEN) ja' propunha `EncodeConfig(max_error_pct=...)` com *"precisao derivada de tolerancia (inovacao)"*, **as 4 tarefas dele seguem DESMARCADAS**; este lab e' o primeiro teste. **ACHADO NOVO**: o `mode` nao e' so' VIES, ele muda a MAGNITUDE do bound e portanto a FORMULA da derivacao (`down` erra 1 passo inteiro, `half-*` erram meio); com a formula de meio passo o `mode=down` prometia 1% e entregava ~1,01% em `wine.density`, e **a verificacao recusou** em vez de entregar dado violando o proprio contrato. **MEDIDO**: `wine.density` com `rel=1%` cai **93,0%** (10137->708 B); `quantum=0,1 + agg=soma` no `retail.UnitPrice` custa **118 B** e derruba o erro da soma de 0,01389% p/ **0,00037%** (37x), o trade explicito que o parametro torna declaravel; `agg` sozinho economiza **0%** (realoca, nao corta, o valor dele e' o CONTRATO); e `rel` e' **inutil em money real** porque a cauda inferior amarra (um item de 0,001 obriga a coluna a 4 casas), o que casa com a norma (ISO 4217 define *minor unit*, nao percentual). **FALTA**: o MARCADOR NO WIRE, hoje o laudo e' objeto Python, e dado ajustado que viaja sem declarar contrato e' indistinguivel de exato. E' a outra metade desta hipotese e pre-requisito de qualquer weld. | aberta (prototipada 2026-08-14; falta o marcador de wire) | taxonomia §4 + nota [`2026-08-14-2010`](../2026-08/2026-08-14-2010-perda-propagacao-de-erro.md) |
| H-LOSS-01 | **Residuo-redistribuido (soma/media/grupo)**: maior-resto/error-diffusion; loss por-linha, soma EXATA. A ideia-chave do owner (parcelamento). **MEDIDO 2026-08-14** (lab `2026-08-14-2145-agg-soma-e-streaming`, 3 formas x 3 casos, 0 falhas), a pedido do owner (*"dependendo da forma que eu peca, tem que ver se ele fica stream compativel"*): sao **TRES implementacoes do MESMO contrato**, e elas diferem exatamente em streaming. **maior-resto** (Hamilton): soma exata, mas **2000 leituras antes de emitir o 1o valor**, prefixo de ENCODER = 100% da fonte, a mesma assinatura algebrica do bN modo `C` e do split embutido, que o repo ja' classificou como *decodavel, nao-emitido-por-default, opt-in*; e e' a linha "buffer > O(1)" que a ADR-0002 refuta mesmo com ganho. **difusao-de-erro** (Floyd-Steinberg 1-D): le 1, emite 1, mantem **1 float** de estado, e a soma sai **exata na escala de d** nos 3 casos, por **+2,0% de bytes** (3028->3090) e **o dobro do erro por linha** (1 passo vs 1/2). **ancora**: a mais barata (**2955 B**) mas e' **OUTRO CONTRATO**, as linhas NAO somam ao total; e a variante trailer, que seria streamavel, ja' esta' REPROVADA (`T-PULSO-SINGLE-COL`). **CORRECAO ao PoC de junho**: registrei que o maior-resto erra 1 passo e o ingenuo 0,5; medido, o maior-resto erra **meio passo** neste dado (ele ordena por residuo e vira quase `half-up`), quem erra 1 passo e' a difusao. **E separa duas nocoes**: o prefixo do DECODER ficou **19 B nas tres formas**, identico, a diferenca e' toda do lado do encoder. Logo `agg` precisa de um SEGUNDO eixo (`agg_forma`), porque "preservar a soma" nao determina o como, e o como decide se cabe num pipe. | aberta (alta); PoC OK; 3 formas medidas 2026-08-14 | taxonomia §1a + poc_soma_preservada.py + lab `2026-08-14-2145-agg-soma-e-streaming` |
...
| H-LOSS-03 | **Round precisao-fixa** (V2-C): casas/sig-figs. Pedra fundamental do vocabulario lossy; ganho nicho ~1.5%. **RESSALVA 2026-08-14**: o PoC de junho reportou bytes **SEM RT VALIDADO** (importou `decode` e nunca chamou), viola o §RT; e seus numeros SINGLE-COL estao obsoletos em **2,5x-13x** porque a rota single-col melhorou desde entao. Re-medido com a checagem no lab `2026-08-14-1745` (os valores JA' ARREDONDADOS atravessam encode/decode identicos): `wine.density` d=2 **10137->708 B (-93,0%)** com soma exata, `retail.UnitPrice` d=1 -18,7% com soma exata. **PRECO QUE O PoC NAO DECLAROU**: o maior-resto erra ate' **1 passo** por linha, o ingenuo ate' **0,5**, preservar a soma CUSTA erro por-linha. | caracterizada (nicho pequeno); re-medida 2026-08-14 | `2026-06-14-v2c-lossy-round-caracterizacao/` + lab `2026-08-14-1745` |
...
| | **PRIMEIRA MEDICAO 2026-08-14** (o owner reabriu com a grafia `0.3(14x0)4` / linha-fantasma). **O nucleo captura ZERO run intra-valor, e e' estrutural**: o OBAT so' compara EXTREMIDADES (`core/online.py:58-71`, sem offset interno em lugar nenhum) e o miolo vai VERBATIM pro wire (`core/online.py:234-236`); o HCC opera sobre ids de atomo, nao chars. Curva medida com ajuste EXATO: 1 valor `bytes = n+11`, 20 distintos `bytes = 20n+69` -> **1,000 B por char repetido por linha, zero amortizacao** ate' n=128. O OBAT tambem NAO dedupa run infixo entre valores distintos. **TETO no corpus: 0,0031%** (11 de 186 colunas tem run>=6, e elas se partem em 2 familias): padding de ID (`Clerk#000000004`, 99% dos casos) o OBAT+HCC **ja' comem como prefixo compartilhado**; cauda de float (`wine.alcohol`, UNICA instancia) e' onde pagaria: **-1,89% na coluna**. **CONTRA-PROVA (medida no lab, e ela mudou de coluna)**: o `o_clerk` NAO custa (-1,70%); quem custa e' o **`c_name`** (**+6,90%**), porque seus 1500 valores sao uma PROGRESSAO ARITMETICA que o seq-RLE esmaga para **87 B**, e colapsar os runs DESTROI a progressao, o run ali nao e' redundancia sobrando, e' o que SUSTENTA outro mecanismo. Break-even em L~6-7. **BLOQUEIOS DA GRAFIA INLINE**: o `*` ja' e' o separador `lit->lit` (`syntax.py:607-617`, byte-load-bearing), `0.3*14*0*4` colide com o parser; e o contador e' digito, que escapa (e' o proprio H-INTRA-02). Desbloqueio = **H-REF-03** (alfabeto por complemento), nao grafia nova. **E os mesmos 40 valores ja' sao atacados pela H-FLOAT-GRAFIA-01**: sao `n/30`, entao o run e' o SINTOMA e a fracao e' a CAUSA. Continua ADIADO, agora com numero. | aberta (medida, adiada) | nota [`2026-08-14-2010`](../2026-08/2026-08-14-2010-rle-intra-valor.md) + **lab** `2026-08-14-2010-rle-intra-valor-medida` |
| H-INTRA-02 | Interacao com o **escape de digito** (`\`): fatorar a repeticao reduz o numero de `\`? Ou o escape come o ganho? Medir o net real. | aberta | n/a |
| H-INTRA-03 | **Overlap** com nature (1) + split estrutural (ADR-0026): o ganho intra-linha generico e' marginal/redundante onde nature ou split ja' atuam? Medir o INCREMENTO antes de welder (anti-incidente 2026-05-21). **Vide tb V2-RLE-STREAM (Pacote 11-bis)**: o achado de 2026-06-19 mostra que a repeticao capturada depende do LAYOUT (runs longos -> tcf-`*N|`; runs curtos skewed -> dict-stream; intra-valor -> H-INTRA), caracterizar H-INTRA ANTES de reabrir V2-RLE (podem se subsumir). | aberta | [V2-RLE-STREAM](../../old/refuted/2026-06-19-v2rle-stream-caracterizacao/result.md) + [estudo](../2026-06/rle-familia-estudo.md) |
| H-NAT-MARK-01 | **Marcador de nature auto-descritivo** no header (tag `cpf`/`cnpj`/`ip` por coluna) pra o `decode` reconhecer a nature SOZINHO. Hoje as natures sao opt-in **OUT-OF-BAND**: o `.tcf` nao diz "esta coluna e' CPF", entao `decode` precisa receber `nature=` (provado: `decode(blob)` sem nature devolve o base-94 cru). O proprio codigo nota "futuro: header carry spec id". Format change (tag `:` SUFIXO do nome no meta-line, `#TCF.7->#TCF.8`, resolucao CORE-ONLY, id desconhecido->cru+flag) -> 0.8. | **DESIGN FEITO -> PARADO em (A)** (owner 2026-06-17): ADR-0027 `proposed`; nao vale o magic permanente AGORA (gate >=15%/2-reais nao bate; DX ja' tem rota zero-core via registry gadget). Revisitar com 2o nature real. | [ADR-0027](../../../../../docs/adr/0027-nature-mark-header-self-describing.md) + [design](../2026-06/f2-nature-mark-header-design.md) |
...
| H-CONST-01 | **Constantes notorias como slot interno** (pi, e/euler, phi, sqrt(2), tau), pedido do owner 2026-08-14: *"tipos notorios como PI e E, no mesmo esquema de dicionario previamente declarado interno. Mas isso pode esperar, so' lembre pra depois"*. **VARREDURA: NUNCA foi discutido** (41 ADRs, 84 tickets, todas as notas, historico do git, zero). **Mas o ESQUEMA e' do proprio owner e ja' esta' registrado 3x**: H-CODEBOOK-01 tier 3 (*"tipos NOTORIOS... a tabela e' senso comum, nao precisa estar no blob"*, 2026-06-24), H-TYPE-07 dict interno (*"petrificado no formato; ate' a referencia fica interna"*, 2026-07-08), e `T-TIPOS-CONFORTO-MAP` (slots `4..13` livres, `14/15` earmarked pelo owner), **e' o slot literal onde pi cairia**. **PRECEDENTE**: `syntax.py:44-47`, os slots baixos vem do FORMATO e NAO viajam; hoje sao 5 tabelas congeladas. **O PROBLEMA QUE O PRECEDENTE NAO COBRE**: as 5 sao **dominio FECHADO de 2-3 valores, fixo POR TIPO**; uma constante e' um valor de **precisao arbitraria**. E a lei do projeto ja' responde, **canonicidade por re-emissao** (5 aplicacoes soldadas, 4 bugs historicos, norma escrita: *"guard de re-emissao e' lei; todo eixo novo nasce com ele"*). Consequencia: **um slot de constante e' UMA GRAFIA EXATA, nao um conceito**, `3.14159` e `3.141592653589793` sao entradas DISTINTAS, e um mapa que casasse "qualquer prefixo de pi" quebraria o RT byte-a-byte. **EVIDENCIA QUE FECHA**: no lab `2026-08-14-1745` o `2.718281828` (Euler truncado) **foi RECUSADO sozinho** pelo guard de re-emissao, a ideia ja' encontrou a lei do projeto uma vez e perdeu. **BLOQUEIOS FORMAIS**: `T-FLOAT-SLOTS` (a ordem canonica dos slots baixos nao esta' fixada) e a pergunta nao-escrita *"o `repr` de float e' contrato de formato?"*, que e' literalmente "quantos digitos". **GATE DE CORPUS**: zero ocorrencias de constante matematica literal em 186 colunas. | **registrada a pedido ("lembre pra depois"), 2026-08-14** | `docs/theory/float-e-variantes-consolidado.md` §8 |
...
| H-NAT-BOOLSTR-01 | **Spec de bool-em-string** (owner 2026-07-17): `'true'`/`'false'` (e grafias) numa coluna STRING ganham um spec que os mapeia p/ o tipo bool interno, **como o CPF**: valor que casa comprime, valor fora do padrao cai em literal (bypass). Owner: "o true/false em string tambem poderia sofrer um spec pra facilitar". **CLASSIFICACAO (medida)**: e' **OTIMIZACAO (.9)**, NAO estrutura, o dado ja' faz RT byte-exato como string (string-core preserva a CAIXA de graca: `'true'`/`'True'`/`'TRUE'` distintos, medido); o spec so' economiza BYTES p/ repetidos. A preocupacao "o spec interno tem que lembrar a caixa (True<>true)" e' resolvida pelo MESMO mecanismo do CPF: `by_status` marca `format_mismatch` -> literal; grafia canonica != interno -> gasta marcador no body mostrando como expande, ainda usando o tipo. Distinto do P2 (`.8H` tag `b`, que e' bool JSON TIPADO, nao string). | `aberta` (`.9`, familia META-TYPE-ENCODERS/natures); confianca: Media (mecanismo ja' existe; ganho a medir) | medicao 2026-07-17 (bypass 5000/5000) |
...
[`rle-familia-estudo.md`](../2026-06/rle-familia-estudo.md)): compete com o RLE de linha `*N|` (tcf, ADR-0016) e
e' tematicamente vizinho do RLE intra-valor (H-INTRA, Pacote 11).

| ID | Hipotese | Status | ref |
|---|---|---|---|
| H-V2RLE-01 | RLE no stream de indices V2-B, uso **GERAL** (tabelas largas / com compressor a jusante). | **CLOSED-INSUFFICIENT-GAIN** (2026-06-19): +1,19% weighted/7 reais, 0/7 >=15%, **-1,39% sob brotli** (some/inverte). | [lab result.md](../../old/refuted/2026-06-19-v2rle-stream-caracterizacao/result.md) |
| H-V2RLE-02 | RLE no stream no **NICHO textual-puro** (payload minusculo, low-card texto **skewed**, ordem natural, sem compressor a jusante). | **ABERTO p/ decisao do owner**: situacao +55%, workclass +22% (2 reais >=15% no nicho); estreito + brotli-fragil. Weld = #TCF.8 + GATE. | [lab forms](../../old/refuted/2026-06-19-v2rle-stream-caracterizacao/result_forms.txt) |
...
| ID | Hipotese | Status | ref |
|---|---|---|---|
| H-GDICT-01 | **Dicionario global no cabecalho**: 1 tabela de unicos no header, todas as colunas referenciam por indice global (dedupe cross-column da tabela). = "cross-column dict" (O-FMT-06/07). | **B1 caracterizado** (2026-06-21, [lab](../../2026-06/2026-06-21/2026-06-21-gdict-caracterizacao/result.md)): GANHA no regime **same-domain-refs** (colunas que referenciam o mesmo dominio: origem/destino, de/para, source/target, FK repetida), **−19.2% textual** + lazy cross-col le' o dict 1x. PERDE em disjunto/entidade (+91%) e partial (+78%) -> **hibrido V2** (dicts por grupo) captura o ganho e evita a perda. **Brotli NAO e' gate** (incompativel com lazy, correcao owner). **B1 PASSA em dado real** (Etapa 5): SNAP ca-GrQc grafo from/to **−19.3% textual** (cruza 15% com 2 cols); OpenFlights airport src/dest −4.6%/−6.6%; lazy cross-col le' dict 1x. Ganho escala com K/N e nº cols same-domain. → ir pro **B2** (#TCF.8 opt-in + hibrido V2 + ADR + GATE). | [lab B1](../../2026-06/2026-06-21/2026-06-21-gdict-caracterizacao/) + [estudo](../2026-06/rle-familia-estudo.md) |
...
completo (ancorado no encoder real) em [`dict-referencia-hipoteses.md`](../2026-06/dict-referencia-hipoteses.md).
...
| ID | Hipotese | Status | ref |
|---|---|---|---|
| H-REF-01 | Unificar os dois sistemas de referencia (reusar `^N` / atom-IDs, nao recriar no V2-B). | aberta (plano) | [doc](../2026-06/dict-referencia-hipoteses.md) |
| H-REF-02 | Indices GLOBAIS/continuos (escopo do blob, nao da coluna) -> habilita dedup cross-column (= H-GDICT). | aberta (plano) | [doc](../2026-06/dict-referencia-hipoteses.md) |
| H-REF-03 | Alfabeto de referencia **livre-de-conflito** automatico (pre-pass acha chars ausentes -> zero escape de digito). Liga a H-INTRA (escape come o ganho). | aberta (plano; **estudar 1o**, barato) | [doc](../2026-06/dict-referencia-hipoteses.md) |
| H-REF-04 | **Modulacao** de modo string<->indice (marcador de MODO amortizado, estilo shift-in/shift-out) em vez de marcar cada ref. | aberta (plano) | [doc](../2026-06/dict-referencia-hipoteses.md) |
| H-REF-05 | Indice de largura minima / curto pros frequentes. **CUIDADO**: encosta em entropy-coding, tende a sumir sob brotli. | aberta (plano, baixa) | [doc](../2026-06/dict-referencia-hipoteses.md) |
...
> [`2026-06-24-0034-filtros-graus-de-entrega.md`](../2026-06/2026-06-24-0034-filtros-graus-de-entrega.md): espectro
...
[`2026-06-16-lazy-query/`](../../../old/welded/2026-06-16-lazy-query/) (`LazyTCF`, reusa decoders core
byte-exato; `where('cidade','SP').sum('valor')` toca so' `cidade`+`valor`). FORA de src/tcf.

| ID | Hipotese | Status | ref |
|---|---|---|---|
| H-QUERY-01 | **View lazy** sobre o blob: `count/sum/min/max/avg` + `where`, com decode por coluna sob demanda (column pruning) + por linha no filtro. Um `decode()` ou gzip/brotli por cima materializaria tudo antes de qualquer conta; o lazy materializa so' o referenciado. | **confirmada-conceitual → PROMOVIDO AO CORE** `../src/tcf/view.py` (T-CODE-LAZY-VIEW-PROMOTE, 2026-06-21; `from tcf import view`; L1-L5). Read-only, le #TCF.7/8; NAO e' versao de formato. (era gadget `scripts/tcf_lazy/`.) | `../src/tcf/view.py` + `../docs/reference/lazy-view.md`; PoC `old/welded/2026-06-16-lazy-query/` |
...
| H-QUERY-05 | **Projecao/filtro de UMA coluna inter-compression como caso de 1a classe** (owner 2026-07-17): `select(col)`+`where(col,pred)`, extrair/filtrar UMA coluna sem materializar as outras, e' operacao DISTINTA (e mais simples) do agrupamento (o sofisticado). Owner: "a gente so' nao viu isso ainda". **NAO e' recurso faltante** (ja' e' o L1/base de H-QUERY-01, welded em view.py: column-pruning + pushdown no stream de indices `@dict`); e' MOLDURA CONCEITUAL, nomear projecao+predicado-de-coluna separado do group-by. A view cobre `#TCF.8M`, single-column e `#TCF.8H` retangular; aninhado, ragged e campo opcional continuam fora porque nao formam tabela retangular. Formalizar o pushdown explicito e' H-QUERY-04. | **capacidade JA welded** (view.py); moldura registrada | `../src/tcf/view.py` + H-QUERY-04 |
| **H-QUERY-04** | **Decode-como-DAG + decode parametrizado + indices escondidos** (design 2026-06-17, owner). Decode = DAG de 5 nos; corte minimo por query so' em `@dict`/raw (tcf entrelacado = fallback total, limite fundamental). Decode unificado = super-metodo `execute(projection,where,agg)` no GADGET (pushdown onde o modo permite); NAO mexer em `decode()` core. Indices escondidos pra grouping: **derivavel > {in-file inerte / sidecar `.tcfx`} > formato**, NUNCA in-blob por default. | **DESIGN FEITO** ([nota](../2026-06/hquery01-decode-dag-indices-design.md)). Plano em fases A/B/C + transversal (ex-E1-E6), barato no gadget (zero core). | `hquery01-decode-dag-indices-design.md` |
...
| H-QUERY-04e | **Fundir filtro e agregacao numa passada** (owner 2026-08-25): hoje `where(...).group_count(col)` faz duas varreduras, uma pra montar os indices e outra pra agrupar. A pergunta: os dois sao complementares, ou existe um caminho fundido (`group_count` com filtro embutido) que percorre uma vez so'? Vale tambem pro `dropna` por filtro, que e' o caso de uso documentado. Hoje a superficie ja' aceita `idx` internamente nas quatro agregacoes, entao a fusao seria de EXECUCAO, nao de API. | aberta (otimizacao) | decidido: funcionalidade primeiro, medir no .9 |
| H-QUERY-04f | **Acesso a' LINHA i sem ler as anteriores, e o remapeamento entre colunas** (owner 2026-08-25): *"se a primeira coluna for filtrada/ordenada para grupo, as chaves poderiam 'pular' nas outras colunas por mapeamento logico"*. MEDIDO: o corpo `@dict` e' tabelinha de K mais stream de LARGURA FIXA, entao a linha i sai por aritmetica (`offset = i * width`), O(1), sem tocar as outras. Provado: 5 posicoes de 2000 lidas com **0 bytes decodificados**. Vale igual no denso (`offset = i * w bits`). No `raw` e' O(i) (achar o i-esimo LF). No **core e' IMPOSSIVEL**: as refs do OBAT sao resolvidas em sequencia, e ler 1 posicao construiu 2000 valores. CONDICAO que a hipotese expoe: o remapeamento so' paga se as OUTRAS colunas cairem em dict/denso, e isso e' decisao do ENCODER, nao do consumidor. Liga direto no P6 de `T-STUDY-USE-PROFILES` (a intencao de consulta influenciando a escolha de modo). Distinto do V2-K do ADR-0018, que e' seek por COLUNA. | aberta (medida, sem weld) | lab 2026-08-25-0600-ordenar-e-remapear |
| H-QUERY-04g | **Ordenar o RESULTADO de um group_by nao e' o mesmo que ordenar linhas** (2026-08-25): as chaves ja' vieram do group_by, sao K, e `sorted` sobre elas le' ZERO byte a mais (medido: 6202 B com e sem, diferenca +0). Nao precisa de API: o consumidor ordena o dict. O caro e' ordenar LINHAS, que e' H-QUERY-04f. Misturar as duas e' o que faz "ordenar" parecer caro sempre. | **resolvida-sem-codigo** | mesmo lab |
| H-QUERY-04h | **O filtro nao estreita o trabalho a jusante** (medido 2026-08-25): `where(f,'sim').group_sum('g','v')` constroi 6000 valores quer o filtro guarde 1% ou 100% das linhas. O `_group_agg` faz `self._col(col)` e SO' DEPOIS aplica os indices, entao a coluna e' materializada inteira antes do corte. Vale pra toda a familia: `.sum()` 4000, `.group_count()` 4000, `.select()` 4000, todos independentes da seletividade. A intuicao (filtrar antes barateia o que vem depois) NAO se confirma hoje. O conserto e' ler so' as posicoes filtradas, que o stream de largura fixa do `@dict` permite por aritmetica (`offset = i * width`), e e' o mesmo mecanismo do H-QUERY-04f. | aberta (medida, sem weld) | lab 2026-08-25-0700, `2-graus.py` |
| **H-QUERY-06** | **Sub-TCF por RECORTE: derivar um blob filho do pai sem `decode` e sem `encode` das N linhas** (owner 2026-08-25: *"extrair um tcf derivado do original pelos filtros (...) decompor um subtcf sem fazer decode,encode, ou view,encode novamente"*). **VIABILIDADE PROVADA NO DOMINIO `#TCF.8M`, COM INDICES VALIDOS E PROJECAO NAO VAZIA**: `raw` recorta por linha, `@dict` fatia o stream e poda a tabelinha de K, `%split` preserva o template e recorre na sub-tabela; o header so' tem os sizes hex reescritos. Core (`tcf`) e' entrelacado = fallback. Podar `@dict` recodifica K, e o fallback recodifica a tabela: a claim e' evitar `decode`+`encode` das N linhas quando a estrutura permite, nao "zero encode". O derivado rapido nao e' necessariamente byte-canonico. | **viabilidade PROVADA, nao welded**; [ticket publico de execucao](../../../../../tickets/T-CODE-VIEW-SUBTCF-RECORTE.md) delimita as bordas. [Lab 1000](../../2026-08/2026-08-25/2026-08-25-1000-subtcf-por-recorte/) prova as primitivas; [lab 1100 `.8M` de uma coluna](../../2026-08/2026-08-25/2026-08-25-1100-recorte-single-col/) prova que poda e' requisito de correcao, **nao a rota single-column real**; [lab 1200](../../2026-08/2026-08-25/2026-08-25-1200-mecanismo-do-recorte/) fecha a peca (20/20 + composicao; rapido em 13/20; diferencial valido 400/400; mediana de bytes +0,0% apos RT). Auditoria de borda: indice fora da faixa/negativo corrompe `@dict`, `cols=[]` gera wire invalido, ordem pedida se perde, single-column real levanta e `.8H` so' cai no fallback. | H-QUERY-05 (projecao) + H-QUERY-04 (pushdown) |
| **H-QUERY-07** | **A consulta fecha sobre BLOB, nao sobre dataset** (owner 2026-08-25: *"ao inves de sair sempre um dataset das views, o objeto intermediario pode ser o blob compactado; ou a view fica latente, por ser lazy, e so' na hora que eu pedir pra sair dataset ou blob"*). Hoje toda consulta termina em valores Python. A ideia e' a view continuar LATENTE ate' o pedido terminal, e existirem DUAS saidas: `to_dataset()` (expande) e `to_tcf()` (recorta, H-QUERY-06). Consequencia: consulta encadeada nunca materializa o intermediario, e `view(view(b).where(...).to_tcf())` fecha o ciclo. Depende de H-QUERY-06 pro lado blob. **A ESTUDAR** (owner: *"nao sei... temos que estudar isso"*): onde o plano latente para de valer (agregador precisa dos valores), e se latencia dupla confunde a superficie. | **registrado, nao estudado** | H-QUERY-06 (recorte) + H-QUERY-04 (decode-como-DAG: o plano latente E' o DAG) |

**Principio transversal H-QUERY (owner 2026-08-26): a `view` e' oportunista no CUSTO,
nao na RESPOSTA.** Buscar a maxima resposta pela menor evidencia suficiente, na ordem que
o modo permitir: header -> estrutura compacta -> K unicos/stream -> posicoes e colunas
pedidas -> materializacao -> fallback. Um atalho so' existe se concordar com o oraculo; se
a menor rota ainda nao foi demonstrada, vai para lab/`.9`, nao vira heuristica. O mesmo
vale para `to_tcf`: projetar antes de tocar, recortar antes de decodificar, e fazer fallback
no menor nivel seguro. No `.8` fecham correcao e contrato; fusao, pushdown e novas rotas
ficam no ciclo de otimizacao. Owners publicos: `DECISAO-GROUPING-SEMANTICA` e
...
[`hquery01-decode-dag-indices-design.md`](../2026-06/hquery01-decode-dag-indices-design.md) §1-9.
...
| H-13-13 | **FECHADA 2026-08-20 -> ticket `T-FMT-CONTRACT-SIGNATURE`**. Medido: de TODOS os kwargs de `encode`, exatamente DOIS produzem wire cujo decode NAO devolve a entrada, `drop_names` (perde os nomes, devolve posicional) e `sort_by` (reordena as linhas), e NENHUM declara isso no wire. O `sort_by` e' o pior: o header fica BYTE-IDENTICO ao normal, logo um wire ordenado e' indistinguivel de um integro. Os demais (`min_header`/`stamp`/`fallback`/`layers`/`min_len`/`parallel`) mudam a representacao mas o decode devolve o original. Assinatura de 4 chars (blake2s->base36, 36^4~1,68M) custaria 4 B/wire. **ATUALIZADO 2026-09-01 (0.8.4)**: duas frases desta medicao cairam, e a que sobrou ficou mais forte. (1) Nao sao mais DOIS kwargs: so' o `drop_names` produz wire cujo decode nao devolve a entrada SEMPRE. O `sort_by` produz AS VEZES, porque o ADR-0050 o tornou candidato: com o proprio exemplo desta linha, `{'nome':[...],'uf':[...]}`, `encode(t)` e `encode(t, sort_by='uf')` dao o MESMO wire de 45 B e o decode devolve `t`. (2) O `sort_by` nao "reordena as linhas": ele AUTORIZA reordenar, e o encoder so' reordena quando isso encolhe. (3) A parte que NAO caiu e' a pior: o wire ordenado continua byte-indistinguivel do integro, e o ADR-0050 mexeu na FREQUENCIA do reordenamento, nao na VISIBILIDADE dele. A pergunta do ticket mudou de "como avisar que reordenei" para "como avisar que POSSO ter reordenado", que e' um bit CONDICIONAL e nao um carimbo fixo (Q6 do ticket). E o escopo do `drop_names` DOBROU: a mesma perda de nomes acontece agora no `#TCF.8R` (`encode(regs, drop_names=True)` volta `[{'0':...},{'1':...}]`), entao a assinatura e' dos NOMES e nao da familia (Q7). | **fechada (virou ticket), premissa parcialmente refutada 2026-09-01** | `tickets/T-FMT-CONTRACT-SIGNATURE.md`; precedente que ja' faz certo: nature `:id` + `_resolve_header_spec` (decoder.py:62-79) |
...
**Custo real do esforco** (medido): nao e' o codigo, sao 3 ajustes pequenos (`split.py:48-56`
...
sem retrabalho. BLOQUEADOR antes de qualquer weld: H-13-06 (grupo x array).
...
| H-14-11 | **Liberar o `sort_by` no `.8R`?** (tensao achada na auditoria de docs de 2026-09-01, entre os DOIS ADRs do mesmo commit). O ADR-0049 recusa o `sort_by` em lista de registros com o argumento de que ele *"reordenaria a lista do usuario calado"*. No MESMO commit, o ADR-0050 tornou o `sort_by` nunca-pior e OPCIONAL: ele so' reordena quando isso encolhe o wire. O argumento da recusa enfraqueceu no dia em que foi escrito, porque "pode reordenar" e' bem menos agressivo que "reordena sempre" | **aberta**, sem lab | Mas NAO e' automatico, e e' por isso que vira hipotese em vez de correcao: o que muda com o FLOOR e' a FREQUENCIA do reordenamento, nao a VISIBILIDADE dele. Numa lista, a ordem continua sendo a unidade que o chamador ve', e continua nao havendo nada no wire que a assine (o header sai byte-identico, ver `T-FMT-CONTRACT-SIGNATURE`). Um reordenamento raro e' pior de depurar que um constante. O caminho honesto, se for liberar, passa por decidir a assinatura primeiro |
| H-14-09 | **A dica previa de agrupamento** (owner 2026-09-01): em vez de ordenar, o encode receberia a informacao de que o consumidor vai AGRUPAR por uma coluna, e escolheria a forma que beneficia o `decode`/`view`, sem mexer na ordem das linhas | **medida, e REDUNDANTE no caso comum** ([lab](../../2026-09/2026-09-01/2026-09-01-0130-entrega-agrupavel-ja-e-o-default/)): o FLOOR ja' entrega a forma agrupavel sozinho em **7 de 8** formas de chave (uf/status/bool/cliente/categoria/mes caem em `@dict`; so' a high-card nao, e la' o dict nem e' candidato nem ha' agrupamento a fazer). A causa e' que os dois criterios sao o MESMO: o que faz o `@dict` vencer em BYTES (poucos distintos repetindo) e' o que faz a coluna ser boa chave de grupo. A garantia vale ate' **K=100 de 200 linhas**, ou seja enquanto cada grupo tiver >=2 linhas | se um dia for feita, o pedido correto NAO e' "ordene", e' *"mantenha esta coluna em `@dict` mesmo que outro candidato seja alguns bytes menor"*: um DESEMPATE no `_best_of` (multi/core.py:560), nao uma rota nova. O ganho a cobrar dela e' MEMORIA |
| H-14-10 | **`sort_by` como FORMA CANONICA** (argumento novo, 2026-09-01): ordenar torna o wire independente da ordem em que o produtor montou as linhas, o que serve dedupe, cache por conteudo, diff e endereco por hash, e e' o que o dict cross-sessao (tier B1) precisaria pra reconhecer trafego repetido | **medida, vale so' com chave UNICA** ([lab](../../2026-09/2026-09-01/2026-09-01-0130-entrega-agrupavel-ja-e-o-default/)): 8 embaralhamentos do mesmo conjunto dao **1** wire com chave unica e **8** com empates, porque o `sorted` e' estavel e as linhas empatadas mantem a ordem de ENTRADA. E chave unica e' exatamente o caso em que agrupar nao serve pra nada | os dois argumentos do `sort_by` **se excluem**: onde o canonico vale, o de agrupamento nao. Este e' proposito OUTRO (dedupe/cache) e a forma correta dele seria ordenar por TODAS as colunas, que e' operador diferente. Conecta com o tier B1 cross-sessao |

**Achado 2026-09-01: o `sort_by` TIRA a chave da forma agrupavel** ([lab](../../2026-09/2026-09-01/2026-09-01-0130-entrega-agrupavel-ja-e-o-default/)). Ordenar agrupa os iguais, o RLE adjacente passa a vencer o `@dict` em bytes, e o `_best_of` troca de candidato (`'#TCF.8M@da=uf,v'` vira `'#TCF.8M23=uf,v'`). So' que o caminho estrutural do `group_count` (view.py:849) existe APENAS no modo `@dict`; nos demais ele cai em `Counter(self._col(col))`, que materializa as N linhas. Ou seja, no eixo que o owner propos (entrega agrupavel), o `sort_by` e' a unica operacao da API que REMOVE a coluna da forma agrupavel, e faz isso em silencio.

**Os 4 vetores ortogonais NAO apontam pro mesmo lado** (medido no mesmo lab, `uf`+contador):
- **MEMORIA**: `@dict` por construcao. O `group_count` responde materializando ZERO linhas (retem K unicos + stream cru) contra N linhas do RLE, em toda a faixa N=200/2.000/20.000. Nenhuma otimizacao apaga isso.
- **VELOCIDADE**: RLE, e a vantagem CRESCE (1,50x -> 5,39x -> 6,51x). Mas a razao e' de IMPLEMENTACAO: o caminho "estrutural" le' MENOS bytes e paga um laco Python por linha (view.py:860), enquanto o outro faz `Counter` em C sobre lista pronta. **Item de `.9`**: vetorizar esse laco inverteria o sinal, e ele e' o gargalo do L3.
- **COMPRESSAO**: dependente do dado (aqui o ordenado ganhou -26,4%, no lab 0010 perdeu +52,1% com companheiras independentes).
- **LATENCIA ao 1o resultado**: NAO medida em lugar nenhum ainda; e' o unico eixo em que o sort ainda poderia ganhar, e exige leitor por LINHA (H-14-07, `.9`).
| H-14-08 | **O FLOOR decide o sort, nao o usuario** (2026-09-01): ao passar `sort_by` o usuario JA' abriu mao da ordem das linhas, entao o encoder ja' esta' AUTORIZADO a reordenar e nada o obriga a reordenar quando isso piora. Tratar a ordenacao como mais um CANDIDATO (do jeito que o `.8M` ja' trata tcf/raw/`@`/`%`): encoda os dois, emite o menor. Vira **nunca-pior** por construcao, e o kwarg passa de "reordene por esta coluna" para "voce PODE reordenar por esta coluna se ajudar" | **SOLDADA 2026-09-01** (ADR-0050, commit da5e3de8; suite 1928 verde) ([lab](../../2026-09/2026-09-01/2026-09-01-0010-sort-by-o-que-ele-compra/)): nos 7 casos do lab evitaria **734 B de perda** e nao perderia ganho nenhum, porque o menor dos dois nunca e' pior que qualquer um deles. Custo: um encode a mais, e so' quando o `sort_by` e' pedido | exige aprovacao I5 + ADR (o `sort_by` nao tem ADR proprio, ao contrario de todos os vizinhos do ciclo de solda: 0022, 0023, 0025, 0026). Supersede a parte "combinatoria" do H-14-03 no caso de UMA chave declarada: aqui nao ha' busca, sao dois candidatos |
...
| H-15-02 | Chooser entre specs de CNPJ | **REMOVIDO pelo ADR-0044**: com UM spec so' nao ha' escolha a fazer. Historico do que se aprendeu: a regra ingenua "tem letra -> alfa" foi MEDIDA e reprovada (erra 8/12; virada em k/n=1/4); a soma-de-payload que a substituiu carregava residuo sistematico de ate' 3,15%; o caso compacto POR VALOR dissolveu os dois problemas | ADR-0042 §4 · ADR-0043 §3 · ADR-0044 |
...
| H-15-07 | Bordas em valor de spec | **FECHADO 2026-08-21 (ADR-0045)**: vazamento do `$` corrigido (`\Z`, 0 divergencia de byte em 9.012 valores) + status `format_bordered` (bytes identicos, telemetria acionavel). O trim preguicoso foi REJEITADO (quebra o RT byte-canonico). A postura LAZY (comprimir + restaurar a borda, ~10 B/valor melhor que literal) fica como estudo `.9`, exige gramatica nova e a prevalencia real e' zero hoje | ADR-0045 · lab `0330` |
` final. Logo `'11.222.333/0001-81
'` classifica como `compressible`, o `
...
| H-15-08 | O LF final do wire, tem ambiguidade? tem necessidade? | **FECHADO 2026-08-21 (lab `0700`): TEM NECESSIDADE.** O par que decide e' `[]` x `['']`: `encode([])` = `#TCF.8\n` e `encode([''])` = `#TCF.8\n\n`. O LF terminador distingue **coluna vazia** de **coluna com um valor vazio**; como SEPARADOR (n-1 LFs) as duas colapsariam no mesmo corpo vazio. Ele carrega **1 bit, so' no caso de borda**, pouco, e suficiente. Pelo criterio do owner (basta nao ter ambiguidade E nao ter necessidade), NAO se qualifica pra remocao. **REVOGA** a conclusao do lab `0500` ("redundante, 55/55"), que media RECUPERABILIDADE (drop+readd ja' sabe que o LF existia) num corpus que omitia `[]`. A assimetria de emissao entre rotas segue precificada como nao-fazer (ADR-0045 §3) | lab `2026-08-21-0700-lf-a-resposta` |

| H-15-09 | **PERFIS POR CAMADA** (direcao owner 2026-08-21: *"nao quero misturar a entrada do dataset e o roundtrip pra construir, a saida em arquivo que possa ser util, e o transporte. tudo e' coisa diferente [...] um flag e um default pra cada situacao [...] so' nao pode atrapalhar o encode/decode"*). QUATRO camadas: C1 entrada · C2 roundtrip (INTOCAVEL) · C3 arquivo · C4 transporte. MEDIDO (lab `0600`): o omissivel do transporte NAO e' o LF (1 B), e' o **CABECALHO (7-8 B)**, que vale **22% a 71%** do wire em payload minusculo (multi 2x2: 21 -> 6 B). E a familia **JA' COMECOU**: `drop_names` (ADR-0029) e' exatamente uma omissao de transporte, e `T-SPEC-SEM-CARIMBO` registra a mesma ideia pro `:id`. Falta o CONCEITO que os une. PROVA de viabilidade: as transformacoes sao FUNCOES PURAS sobre o wire, inversa exata, 7/7 casos, **0 linha de encode/decode tocada**. | aberta, desenho com numeros, NAO weld. **RESSALVA CRITICA**: a tabela e' o TETO do omissivel, nao ganho liquido, o contrato fora de banda so' compensa se AMORTIZADO (WebSocket/gRPC/MQTT/envelope), nunca em HTTP avulso | lab `2026-08-21-0600-quatro-camadas` |
...
[2026-07-16-2321-revisao-generosa-hierarquia.md](../2026-07/2026-07-16-2321-revisao-generosa-hierarquia.md) §4.
...
[estrutura-sem-dado-levantamento.md](../2026-07/estrutura-sem-dado-levantamento.md). Nota: as entradas 2026-07-14/15
...
- **ESTUDO TCF-HIERARQUICO (grupo de pecas, owner 2026-07-05)**: como representar documento JSON aninhado
  ...
  [`estudo-tcf-hierarquico-mapa.md`](../2026-07/estudo-tcf-hierarquico-mapa.md); guarda-chuva T-STUDY-HIERARCHICAL-TCF.
  ...
  (FD) · P8 teoria-cardinalidade (forca + rapido-vs-pleno). Teoria: [`teoria-cardinalidade.md`](../2026-07/teoria-cardinalidade.md).
...
- **H-NEST-01** (JSON aninhado / "TCF aninhado similar ao JSON", owner 2026-07-05): o TCF e' tabular; o
  ...
  virou HCC). Lab [`2026-07-05-nested-tcf-study/`](../../2026-07/2026-07-05/2026-07-05-nested-tcf-study/result.md).
...
- **H-TYPE-01** (fidelidade de tipos no TCF.8H, owner 2026-07-06, Ciclo 1a): o codec hierarquico faz
  ...
  all-string. Lab [`2026-07-06-2221-tcf8h-fidelidade-tipos/`](../../2026-07/2026-07-06/2026-07-06-2221-tcf8h-fidelidade-tipos/result.md).
  ...
  - **Formas (1b)**: aninhamento-fundo + array-vazio FECHAM RT (funcionalidade de graca); chave-ausente +
    ...
    posicional (Ciclo 1c). Lab [`2026-07-06-2238-tcf8h-escala-formas-e-tipos/`](../../2026-07/2026-07-06/2026-07-06-2238-tcf8h-escala-formas-e-tipos/result.md).
  - **Reframe TIPOS COMO SPECS (owner 2026-07-06)**: tipo/hex/natures = MESMO espectro de specs (minimo->rico).
    ...
    Nota [`tipos-como-specs.md`](../2026-07/tipos-como-specs.md) + lab [`2026-07-06-2310-tipos-como-specs/`](../../2026-07/2026-07-06/2026-07-06-2310-tipos-como-specs/result.md).
  ...
  - **Revisão (owner 2026-07-15, ciclo H-SUBST-INDEX-01)**: **bool** provavelmente migra da tag C-híbrida
    ...
    (cardinalidade infinita não reserva índice). Ver [substituicao-indices-especiais-plano.md](../2026-07/substituicao-indices-especiais-plano.md).
...
- **H-SUBST-INDEX-01** (índices de substituição p/ especiais via dicionário pré-semeado, owner 2026-07-15):
  ...
  medição pendente). Plano: [substituicao-indices-especiais-plano.md](../2026-07/substituicao-indices-especiais-plano.md).
...
- **H-DATASETH-COMPLETE-01** (capacidade semântica antes do wire, owner 2026-07-16): um modelo
  ...
  [`2026-07-16-1708-dataseth-s0-s3-semantica-vinculos/`](../../2026-07/2026-07-16/2026-07-16-1708-dataseth-s0-s3-semantica-vinculos/result.md):
...
- **H-HIER-SCALAR-01** (escalares especiais no DatasetH, owner 2026-07-13): `null`, `NaN`, `+Infinity` e
  ...
  **Update 2026-07-13**: o lab
  [`2026-07-13-1921-dataseth-typed-header-domain/`](../../2026-07/2026-07-13/2026-07-13-1921-dataseth-typed-header-domain/result.md)
  ...
  P4 ainda aberto. Plano e falsificadores: [dataseth-hierarquia-completa-plano.md](../2026-07/dataseth-hierarquia-completa-plano.md).
...
- **H-DISC-ACCEL-01** (discriminador como DICA de aceleração + mimemagic, owner 2026-07-14): os chars do
  ...
  **baixa prioridade** → NÃO implementar agora; só **preservar a separação** (não "soldar" demais as peças).
  ...
  [tcf8-header-char-registry](../2026-07/tcf8-header-char-registry.md) + [tcf-camadas-arquitetura](../2026-07/tcf-camadas-arquitetura.md).
...
- **H-ENCODE-DEADLINE-01** (desistência de encode por latência: pulsos/anytime, owner 2026-07-16):
  ...
  Baixa. NÃO é `.8`. Registro: [contrato-externalizado-e-aceleradores.md](../2026-07/contrato-externalizado-e-aceleradores.md).
...
- **H-STRUCT-DEF-01** (formas opacas → DEFINIÇÕES de wire, não compressão, owner 2026-07-16: família
  ...
  [estrutura-sem-dado-levantamento.md](../2026-07/estrutura-sem-dado-levantamento.md).
...
- **H-STRUCT-AMORT-01** (estrutura repetida amortiza via contrato, 2026-07-16): num stream de N docs com o
  ...
  contrato). Levantamento: [estrutura-sem-dado-levantamento.md](../2026-07/estrutura-sem-dado-levantamento.md).
...
- **H-STRUCT-META-01** (o próprio meta é comprimível?, 2026-07-16, `.9`): o framing profundo repete
  ...
  Levantamento: [estrutura-sem-dado-levantamento.md](../2026-07/estrutura-sem-dado-levantamento.md).
...
- **H-PROFILE-JSONLIKE-01** (perfil "json-like" como política de ADAPTADOR, owner 2026-07-17): owner
  ...
  **Veredito**: das 10 bordas **exatamente 1 vira parâmetro**, e vive no ADAPTADOR (texto→DatasetH),
  ...
  Condições + teste: [perfil-json-like-condicoes-parametro.md](../2026-07/perfil-json-like-condicoes-parametro.md).
...
- **H-HIER-SIDEOUTPUTS-01** (canal de efeito colateral no `.8H`: **pré-requisito de qualquer
  ...
  **hoje um warning no `.8H` não tem onde morar**. Precedentes: `warnings.warn(UserWarning)` em
  ...
  Fonte: [perfil-json-like-condicoes-parametro.md](../2026-07/perfil-json-like-condicoes-parametro.md) §4.
...
- **H-HIER-EMASK-SPARSE-01** (emask densa global → por-instância/esparsa, 2026-07-16, achado da
  ...
  **um** array liga emask O(total-elementos) pro dataset inteiro; e null periódico não-adjacente não
  ...
  Fonte: [2026-07-16-2321-revisao-generosa-hierarquia.md](../2026-07/2026-07-16-2321-revisao-generosa-hierarquia.md) §2.2b.
...
- **H-HIER-FANOUT-SPLIT-01** (fan-out FIXO deduzido → desaninhar em k colunas irmãs, 2026-07-16):
  ...
  **medido**: telemetria 200×`{'t':i,'v':[1.5,2.5,3.5]}` → folhas 1804 B = **96,5% do wire**
  ...
  Fonte: [2026-07-16-2321-revisao-generosa-hierarquia.md](../2026-07/2026-07-16-2321-revisao-generosa-hierarquia.md) §2.2c.
...
- **H-HIER-PREP-RUNS-01** (preparação emite runs, não listas densas, 2026-07-16, **`.9`**): a
  ...
  funcionalidade). Fonte: [2026-07-16-2321-revisao-generosa-hierarquia.md](../2026-07/2026-07-16-2321-revisao-generosa-hierarquia.md) §2.2a.
...
- **H-TYPE-02** (bit-packing por largura `bN` p/ enum/bool baixa-cardinalidade, owner 2026-07-07): familia
  ...
  [`2026-07-08-1938-bn-gate-realworld-5fontes`](../../2026-07/2026-07-08/2026-07-08-1938-bn-gate-realworld-5fontes/result.md).
  ...
  [2026-07-06-2354-spec-bin-motor](../../2026-07/2026-07-06/2026-07-06-2354-spec-bin-motor/result.md) (secao CORRECAO),
  [2026-07-07-0028-spec-bitwidth-bN](../../2026-07/2026-07-07/2026-07-07-0028-spec-bitwidth-bN/result.md) (secao CORRECAO).
  ...
  existente, nao inaugura o problema. Consolidacao completa: [tipos-como-specs.md](../2026-07/tipos-como-specs.md)
  (secao "CONSOLIDACAO E CORRECAO 2026-07-07").
  - **Prior-art ADICIONAL (achado na reorganizacao 2026-07-08)**: [`specs-capacity-map.md`](../2026-06/specs-capacity-map.md)
  ...
  - **Posição no pipeline (owner 2026-07-15, ciclo H-SUBST-INDEX-01)**: bN é o **estágio 3** do pipeline
    ...
    agora). Ver [substituicao-indices-especiais-plano.md](../2026-07/substituicao-indices-especiais-plano.md).
...
- **H-TYPE-04** (bN como candidato do min() por-coluna, PROJETADA, owner 2026-07-07): bN encaixa como mais
  ...
  [`tipos-meta-grupo-fluxo.md`](../2026-07/tipos-meta-grupo-fluxo.md). Meta-grupo: **H-TYPE-00** (round-trip = regra
...
- **H-TYPE-05** (bN default sob guarda-chuva de PERFIL DE COMPRESSAO, owner 2026-07-07, ABERTA/ver-depois):
  ...
  `confianca: Baixa`. Design: [`tipos-meta-grupo-fluxo.md`](../2026-07/tipos-meta-grupo-fluxo.md) §8.
...
- **H-TYPE-07** (dicionario INTERNO classico + duas perspectivas do bN, owner 2026-07-08, PROPOSTA):
  ...
  (byte NAO justifica)**: o [outer-dict/codebook](../2026-06/cep-outer-dict-codebook-pesquisa.md) ja' achou que codebook
  ...
  (owner). Detalhe: [`bn-dict-perspectivas-e-dict-interno.md`](../2026-07/bn-dict-perspectivas-e-dict-interno.md).
  ...
  - **F1 MEDIDO** ([lab 2026-07-08-2302](../../2026-07/2026-07-08/2026-07-08-2302-f1-bypass-latencia/result.md)): bypass
  ...
  - **F3 MEDIDO** ([lab 2026-07-08-2355](../../2026-07/2026-07-08/2026-07-08-2355-f3-bn-seletivo/result.md), verificado por
...
- **H-TX-01** (gate de posicionamento de transmissao, pendente desde 2026-06-21): TCF+brotli vs
  **NDJSON+brotli** (o concorrente textual real, nao so' CSV). Lab
  [`2026-07-05-t1-ndjson-brotli/`](../../2026-07/2026-07-05/2026-07-05-t1-ndjson-brotli/result.md), 6 datasets reais ×
  ...
  - **Perfil duplo upload/download** (insight owner): download = volume; payload **cadenciado**
    ...
    atualizada em [`transmissao-api-onde-tcf-importa.md`](../2026-06/transmissao-api-onde-tcf-importa.md).
