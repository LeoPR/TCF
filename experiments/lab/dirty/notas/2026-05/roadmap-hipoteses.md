...
| H-15-07 | **Bordas em valor de spec** (reenquadrado pelo owner 2026-08-21: *"similar a se comportar como um trim de bordas"*). REAVALIADO em 4 eixos (lab `0330`): **(E1)** o TCF **nao faz trim** — 9 de 10 variantes de borda (espaco, tab, CR, CRLF, LF duplo, LF a esquerda) caem em literal com RT correto; a unica excecao e' o **LF unico a direita**, vazamento acidental do `$` da regex. Nao e' politica: e' um caractere escapando. Raio: CPF (todos), CNPJ (todos), IP nao-padded; **data-iso ESCAPA porque checa o COMPRIMENTO** — a defesa que os outros nao tem. **(E2)** prevalencia ZERO em 20.000 linhas reais, mas plausivel em INGESTAO (`for line in f:` sem strip produz exatamente o caso que vaza). **(E3)** as 3 posturas por valor: rigido/literal 20 B (RT ok) · preguicoso/trim 7 B (**PERDE** — e' o bug de hoje) · lazy c/ restauro 10 B (RT ok, gramatica NOVA). **(E4)** a guarda `v != v.strip()` custa **-4,2% = ruido** em 50.000 valores limpos: nao ha' custo a tolerar no caminho comum. | aberta — **3 pecas separaveis**: (1) fechar o vazamento `$`->`\Z`, uniformiza, 0 divergencia de byte em 9.012 valores, **toca 3 specs = precisa aprovacao**; (2) rotulo `format_bordered` na telemetria (bytes identicos; precedente `format_unmasked`/`format_padded_zeros`); (3) postura lazy = estudo `.9` (gramatica nova). NAO recomendado: trim preguicoso como default — quebra o RT byte-canonico; se um dia, flag explicita | lab `2026-08-21-0330-bordas-em-spec` |
