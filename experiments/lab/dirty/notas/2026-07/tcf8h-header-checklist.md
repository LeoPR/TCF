...
- [ ] **marcador de tipo na divergência** — quando o tipo diverge de string, uma **letra escrita** no colchete (`i`/`f`/`b`/`n`/`s` colada no size: `idade:4i`) dá fidelidade de tipo; elemento C1, só emitido na colisão. **Prototipado + medido (Ciclos 1a/1b, RT)** — [1a](../../2026-07/2026-07-06/2026-07-06-2221-tcf8h-fidelidade-tipos/result.md) · [1b](../../2026-07/2026-07-06/2026-07-06-2238-tcf8h-escala-formas-e-tipos/result.md); H-TYPE-01. **Estratégia decidida (1b, do número): C-híbrida** — deduz número/bool de graça, tag só a colisão (string-ambígua→`s`, null→`n`); **A-explícita** (tudo taggeado) é fallback quando strings-ambíguas dominam. Análogo do hex-default (C4). Custo: folha DFS-última tipada perde a última-sem-size (liga com C5). *(dirty proto; report.md:33)*
...
- [ ] **header-derivável-por-contrato (O-FMT-14)** — esquema pré-acordado → header colapsa pra assinatura (~6B) ou 0B out-of-band; maior lever restante, custa auto-descrição; só paga em payload minúsculo. *(header-minimal/result.md)* — é a **categoria "supressível-c/-declaração"** do contrato de omissão [T-FMT-OMIT-OR-DECLARE](../../../../../tickets/T-FMT-OMIT-OR-DECLARE.md) (pré-1.0): suprimir até o magic exige declaração no encode+decode + fail-loud + proveniência. **Distinto** de hex-default (convenção-default, auto-descritivo).
