...
- **Posição no disco** (aninhamento 2026-07-22): `experiments/lab/dirty/<YYYY-MM>/<YYYY-MM-DD>/<nome-do-lab>/`.
  A data já está no nome; os 2 níveis de pasta só evitam uma `dirty/` flat com 60+ macros. Ao criar um lab
  novo, crie/reuse esses 2 níveis ANTES — o nome do lab **não** muda. `old/` tem layout próprio
  (welded/refuted/obsolete), **não** aninhado por data.
- **Notas** (`dirty/notas/`), duas naturezas — só a 2ª leva data-hora:
  - *registry/referência vivo* (continuamente atualizado, referenciado por nome em toda parte:
    `roadmap-hipoteses.md`, `tcf8-estrutura-plano.md`, `specs-capacity-map.md`, `fila-estudos-tcf8.md`, mapas,
    inventários, checklists) → **nome-nu ESTÁVEL, sem data** (datá-lo pelo 1º commit mente — a nota vive até hoje;
    e renomear quebra as referências por nome). O horário preciso vive no header/frontmatter + git.
  - *one-shot datado* (registro de UMA sessão que não se revisa: parecer, revisão, dúvidas, diagnóstico,
    assessment, metodologia de avaliação) → `YYYY-MM-DD-HHMM-descricao.md` (mesma forma dos labs, ordenável).
  - Notas ficam agrupadas por `<YYYY-MM>/` (mês do 1º commit); `diario/` e `checkpoints/` fora do agrupamento.
...
- Gabarito desta estrutura: `experiments/lab/dirty/2026-07/2026-07-13/2026-07-13-2019-especiais-formatos-lado-a-lado/`.
...
`experiments/lab/dirty/2026-07/2026-07-05/2026-07-05-1509-tcf-hierarquico-tabelao-vs-2tabelas/` (e `2026-06-27-gdict-b2-prototype/`):
