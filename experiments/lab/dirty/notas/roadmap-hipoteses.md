...
| H-RW-05 | Encode tem complexidade quadratica O(N²) | **confirmada → MITIGADA** (alpha 1.75 → 1.42 pos-ADR-0009) | EXP-014 (lineitem 1k-20k) | Lineitem full 60175 = 71min → 18.5min. Trigrama index welded em src/tcf 2026-05-19. |
...
| H-PERF-04 | Trigrama de meio dispersa buckets em colunas datetime | aberta | nao testada | Datas TPC-H tem buckets grandes (prefixos `199/200/202`). `s[len(s)//2-1:len(s)//2+2]` pode dispersar. Esperado 5x+ em l_shipdate. |
| H-PERF-05 | Otimizar HCC `_detect_compositions` (24% tempo restante) | aberta | nao testada | Sub-exp 01 mostrou HCC como 2o hotspot. Pos-OBAT-opt, vira gargalo relativo. DP ou estrutura indexada. |
