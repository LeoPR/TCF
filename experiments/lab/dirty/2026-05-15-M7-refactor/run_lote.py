"""run_lote.py — M7 (refactor + nova estrutura de debug).

Layout:
  resultados/
    tokens/<dataset>.txt          alg16 raw (compartilhado entre micros)
    matriz_bytes.csv
    matriz_comparativa.md
  <micro>/
    output/<dataset>.tcf
    decoded/<dataset>.csv
    debug/<dataset>.txt           INPUT + TCF + DECODE (sem tokens)
    detector_trace/<dataset>.txt  iteracoes do detector (se aplicavel)
    redes/<dataset>.txt           estado da rede (atomos + composicoes)
"""
...
def gerar_tokens_raiz(nome_ds, unicas, tokens, frags_por_no, tokens_dir):
    """Salva alg16 raw tokens em resultados/tokens/<dataset>.txt
    (compartilhado entre micros)."""
    ...
    (tokens_dir / f"{nome_ds}.txt").write_text(
        "\n".join(out), encoding="utf-8")


def rodar_micro(syn, nome_ds, linhas, unicas, tokens,
                  header, micro_dir):
    """Encode + decode + gera output/decoded/debug/detector_trace/redes."""
    ...
    try:
        ...
        (micro_dir / "debug" / f"{nome_ds}.txt").write_text(
            f"ENCODE FAILED: {e}\n", encoding="utf-8")
        return {"ok": False, "stage": "encode", "err": str(e),
                "bytes": -1}

    (micro_dir / "output" / f"{nome_ds}.tcf").write_text(
        tcf, encoding="utf-8")
    ...
    # debug/<dataset>.txt: INPUT + TCF + DECODE (sem tokens; tokens em resultados/tokens/)
    ...
    # detector_trace/<dataset>.txt: se syntax provê
    if hasattr(syn, 'get_trace') and syn.get_trace():
        (micro_dir / "detector_trace" / f"{nome_ds}.txt").write_text(
            syn.get_trace(), encoding="utf-8")

    # redes/<dataset>.txt: se syntax provê
    if hasattr(syn, 'get_rede') and syn.get_rede():
        (micro_dir / "redes" / f"{nome_ds}.txt").write_text(
            syn.get_rede(), encoding="utf-8")
    ...
    with (base_out / "matriz_bytes.csv").open(
        ...
        for ds in datasets:
            ...
            menor = (min(bytes_por_syn, key=bytes_por_syn.get)
                     if bytes_por_syn else "(nenhum)")
            row.append(menor)
            w.writerow(row)

    md = ["# M7 — Matriz comparativa (refactor)",
           "",
