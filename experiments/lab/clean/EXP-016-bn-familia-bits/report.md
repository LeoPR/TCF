...
|---|---|---:|---:|:-:|:-:|---|
| `bool-nativo` | tipado-b | 47 | n/a | recusa | OK | bool Python puro: o modo denso `b1` tem domínio IMPLÍCITO e deve vencer o bN |
| `bool-nativo-null` | tipado-b | 79 | n/a | recusa | OK | com null o denso `b1` não se aplica; quem cobre é o `b2`/lazy |
| `bool-constante-true` | tipado-b | 16 | n/a | recusa | OK | k=1: o core resolve com RLE; o bN nem se qualifica |
...
| `int-01` | bN-B-tipado-n | 55 | n/a | ativa | OK | `0`/`1` como int: rota tipada `n` COM bN (weld T-BN-TIPADO), 608 B viraram 55 |
...
| caso | rota | bytes | sem bN | espera | veredito | por que existe |
|---|---|---:|---:|:-:|:-:|---|
| `n-zero` | core | 7 | n/a | recusa | OK | coluna vazia: `[]` tem grafia própria (`#TCF.8\n`) |
...
| caso | rota | bytes | sem bN | espera | veredito | por que existe |
|---|---|---:|---:|:-:|:-:|---|
| `float-simples` | bN-B-tipado-n | 93 | n/a | ativa | OK | float k=3 na rota tipada `n` com bN: a grafia canônica vira domínio |
| `float-integral` | bN-B-tipado-n | 59 | n/a | ativa | OK | float que parece int no `repr` |
| `float-neg-zero` | bN-B-tipado-n | 96 | n/a | ativa | OK | `-0.0 == 0.0` em Python: só o `copysign` distingue |
| `misto-int-float` | bN-B-tipado-n | 98 | n/a | ativa | OK | int e float na MESMA coluna |
| `bool-vs-int` | n/a | n/a | n/a | qualquer | fail-loud | `True == 1` em Python. FRONTEIRA DECLARADA: união bool+int no mesmo slot está fora do `.8H` (ratificada 2026-07-17), tem de falhar alto, não deduplicar em silêncio |
| `int-grande` | bN-B-tipado-n | 74 | n/a | ativa | OK | int além de 64 bits |
| `nan` | n/a | n/a | n/a | qualquer | fail-loud | NaN: fora do JSON (RFC 8259), deve FALHAR ALTO |
| `inf` | n/a | n/a | n/a | qualquer | fail-loud | ±Inf: idem |
