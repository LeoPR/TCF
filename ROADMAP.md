...
| **H-QUERY-04** | Expansão (design 2026-06-17): **decode-como-DAG**, decode parametrizado (`execute()` pushdown), **índices escondidos** pra grouping | M | nenhum (gadget) | **DESIGN FEITO** ([nota](experiments/lab/dirty/notas/hquery01-decode-dag-indices-design.md)). Princípio: índices = **derivável > sidecar `.tcfx` > formato**, nunca in-blob por default. Plano E1-E6 barato no gadget. Limite duro: coluna `tcf` é entrelaçada → fallback total (o lazy vive em `@dict`/raw). |
