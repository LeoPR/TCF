...
> | **`T-SPEC-PARSE-X-ALVO`** | "8 specs" vira "1 spec, 8 parsers" | separar o PARSE do ALVO no spec: N grafias -> data canonica -> N alvos, com o `min()` escolhendo o alvo por regime (medido: nenhum alvo ganha sempre — delta-dias 5/8, ordinal-denso 2/8, iso 1/8). Dissolve o atrito "CPF tem 1 grafia, data tem muitas". NAO fazer agora: so' existe UMA grafia soldada, e separar sem necessidade e' abstracao especulativa |
> | **`T-DATA-ALVO-DELTA`** | delta-coluna: **2-11x** onde o ciclo nao e' exato; robusto a ruido | **MEDIDO 2026-08-09** (lab `0042-data-alvo-delta`, RT 12/12): transform de coluna `[1o ordinal, depois deltas; invalido → _literal]` compoe com o core inteiro — alfabeto pequeno cai no bN (`mensal` 1085→349, `quinzenal` 3951→349, `espalhado-ordenado` 4059→644), feriado vira SIMBOLO e nao quebra (345-353 B sob todo ruido testado). COMPLEMENTAR ao periodico (cada um ganha onde o outro nao alcanca; juntos sob FLOOR cobrem tudo). PRESSUPOE `T-NATURE-CANDIDATO-BN`. Aguarda decisao de design do owner (protocolo da nature: transform de coluna) |
> | **`T-SEQRLE-PERIODICO`** | ciclo EXATO paga UMA vez: `uteis` 1590→**41 B** (39x); n=6000→**42** (O(1)); ids nao-data 1959→**33** (nivel CORE, sem nature) | ideia do owner (anterior a esta rodada), **MEDIDA 2026-08-09** (lab `0042`): delta ciclando ENTRE linhas (`*600~1,3,1,1,1\|739617` e' o wire INTEIRO de 600 dias uteis). Quebra a cada perturbacao (feriado-mensal 649 vs 345 do delta-coluna) — dai a complementaridade. **SONDA DE DESIGN 2026-08-09** (`design_probe.py`, monkeypatch de `HCCSeqRLE` — `src/tcf` intocado, `encode`/`decode` reais): o mecanismo posto DENTRO do `compact_body` entrega 1590→**40 B**, n=6000→**41** (381x), ids nao-data→**32** (61x), e **D1-D9 = 1545 + real-world = 89430 byte-IDENTICOS** aos congelados; decoder de hoje diante do wire novo **falha alto** (E3); valores adversariais que imitam o marcador fazem RT (E1). **CUSTO: UM arquivo** (`hcc_seqrle.py`, ~70 linhas, encode/decode espelhados) — `_contador_declarado` e o FLOOR por corpo ja' servem. CORRECAO: o `!!` NAO e' rota raw, e' o sufixo de POLARIDADE (camada de borda pos-encode) — o corpo interno tem escapes, entao `compare_for_seq`/`shift_escape_digits` servem SEM espelho novo. DUAS GUARDAS obrigatorias (a sonda pegou as duas): rejeitar pad uniforme (`[1,1]` = `*N+d\|` disfarcado, piorava o diario 32→34) e FLOOR contra o corpo JA' COMPACTADO, nao o cru (piorava 4 de 8). **SINTAXE DECIDIDA** (owner 2026-08-09): fica `~`; se colidir, troca antes do 1.0 — marcador e' abstrato por dentro, o char e' so' a saida. **CUSTO DE CPU MEDIDO** (era "a medir"): o detector ingenuo e' **O(n²)** — 13,8 s em n=2400 contra 47 ms do encode; tres consertos (fronteira de cadeia 1x, saida curta, salto de padrao uniforme via pre-calculo O(n)) levam a **+35%** no caso em que o mecanismo NAO ganha, com bytes IDENTICOS e gates intactos. Sobra a compartilhar: o array de deltas que o `detect_seq_runs` ja computa (6,8 ms contra 1,6 ms da logica de periodo) — **compartilhar e' parte do weld**. ADR-0040 escrito (proposto); codigo do weld pronto pra revisao em `weld_proposto.py`. **RECOMENDADO como o proximo weld**; aguarda o "pode soldar" |
> | **`T-NATURE-CANDIDATO-BN`** | 19/70/**298 B** (k=5/12/60) | o CANDIDATO da nature nao consulta o bN (mesma classe do fix do baseline em 2026-08-08): a coluna transformada vai pro `_encode_column` cru. Weld pequeno; **aguarda aprovacao do owner** (mexe em src/tcf). Medido na triagem `0024` (H1) |
