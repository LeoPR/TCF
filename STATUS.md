...
> | **`T-SEQRLE-PERIODICO`** | ciclo EXATO paga UMA vez: `uteis` 1590→**41 B** (39x); n=6000→**42** (O(1)); ids nao-data 1959→**33** (nivel CORE, sem nature) | ideia do owner (anterior a esta rodada), **MEDIDA 2026-08-09** (lab `0042`): delta ciclando ENTRE linhas (`*600~1,3,1,1,1\|739617` e' o wire INTEIRO de 600 dias uteis). Quebra a cada perturbacao (feriado-mensal 649 vs 345 do delta-coluna) — dai a complementaridade. ACHADOS de design: forma degenerada de 1 ciclo = LISTA de deltas, perde do delta-coluna, exigir >=2 ciclos; rota do candidato digit-heavy e' o raw `#TCF.8!!` (precisa espelho raw; `compare_for_seq` e' so' do modo escapado); virgula ja' e' do per-run ADR-0016 e `~` e' operador composicional → sintaxe a decidir; format change #TCF.8. Controles uniformes byte-identicos. Aguarda decisao do owner |
