...
> | **`T-DATA-LAZY-ISO`** | até **−99,5%** em coluna regular; **−22,7%** até sem regularidade nenhuma | pre-tx de data no molde da nature do CPF (per-valor, literal com 1 char). Alvo = o `*N+M|` do seq-RLE, que JA' existe. Prototipado e medido no lab `2026-08-08-0016`: **RT 19/19** incluindo spec errando 100%, sujeira 50%, ambiguidade BR×US, 29/02 inexistente, ano 1 e 9999. Piso do prejuizo **+4,9%** (marcador de 1 B/valor). **Ambiguidade NAO precisa ser resolvida**: a transformacao tem de ser INVERSIVEL, nao CORRETA — chute errado custa bytes, nunca dado. Falta: custo de declarar o spec no wire (precedente `#TCF.8 :cpf`, ADR-0027) |
