...
> | **`T-DATA-LAZY-ISO`** | até **−99,6%** onde o OBAT é fraco; **0%** onde ele já resolve | pre-tx de data no molde da nature do CPF. **Detecta por `date.fromisoformat` (299 ns/valor, = olhar 3 chars, 44x mais barato que `strptime`) + guard de re-emissao**. Entra como CANDIDATO do min(), NUNCA default — medido: no regime `agrupado` o spec PIORA 123%, porque destroi o RLE que o OBAT explorava. RT 19/19 com spec errando 100%; teto do prejuizo +4,9%; ambiguidade custa 0% com FLOOR. Consolidado: nota `2026-08-08-data-consolidado` |
