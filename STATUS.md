...
> | ~~**`T-POLARIDADE-COME-NOME`**~~ **SOLDADO 2026-08-16** (opcao B, aprovada pelo owner; item C1 da fila do M) | **FIX**: escopo de discriminador em `decoder.py` — o pre-passe de polaridade so age quando o disc NAO e `M`/`H`. **A PREMISSA foi MEDIDA antes de mexer**: o encode nunca polariza essas rotas — declarado em `encoder.py:489` (*"`.8M`/`.8H`/spec ficam de fora deste weld"*), os 3 sitios de `polariza()` sao single-col, e o lab mediu **2.000 wires `.8M` + 2.000 `.8H`, ZERO com sufixo separavel**; logo rodar o pre-passe ali so PODIA errar. **ANTES/DEPOIS do mesmo `run.py`**: `.8M` **48/64 -> 0/64**, `.8H` **38/64 -> 0/64**, variacoes novas **3/19 -> 0/19**, contra-prova de 2 colunas 64/64 nas duas, 0 warnings nas duas. **WIRE BYTE-IDENTICO**: os headers das 19 variacoes e dos 5 controles single-col nao mudaram (0 diferencas entre as rodadas) e os gates byte-canonical seguem verdes — **custo em bytes: ZERO**. **A OPCAO A (escapar o nome no emissor) FOI DESCARTADA**: mudaria bytes, exigiria re-pinar baseline, e trataria o sintoma deixando a polaridade lendo header que nao e dela. **VARIACOES NOVAS pedidas pelo owner antes do weld**: (a) **Unicode e SEGURO** (`obs°`/`valor€`/`medida±`/`temp℃`/`ção…` sempre fecharam — a FAIXA e ASCII 0x21-0x7E), o que **resolve o vies "so ASCII" que o lab declarava**; (b) **3 casos novos do MESMO defeito** que o sweep `ab`+pontuacao nao alcancava (nomes `.`, `..`, `a.`); (c) `min_header=False`, 3 colunas, spec `:dt`, `.8H` aninhado e os modos `!`/`@`/`%` **ja eram seguros** (o meta ganha `=`/`,`/`:`/prefixo e o `tag.isalnum()` do separador falha — a conservadoria que ja existia); (d) `drop_names` era **erro do meu teste**, nao defeito (nomes viram posicionais por design, ADR-0029 — a prova e por VALORES). **PINO**: `TestPolaridadeComeNome` (16 casos) em `test_f0_boundary_fixes.py`; verificado por `git stash` do fix que **13 dos 16 falham sem ele**, e os 3 que passam nos dois sao as contra-provas. Suite **1269 -> 1285 passed**. Lab: `1330-polaridade-come-nome-de-coluna` (autodetecta o estado; grava `resultado-antes.json` e `resultado-depois.json`) | *(registro original)* **RT QUEBRADO CALADO** — o dado sai do round-trip diferente do que entrou, sem warning | **A PIOR CLASSE DO PROJETO**, achado no mapeamento de M/H e reproduzido em lab (`1330-polaridade-come-nome-de-coluna`). `decode(encode({"obs.": [5 valores]}))` devolve a chave **`"obs"`**; com pontuacao DOBRADA (`"obs.."`) **os VALORES tambem corrompem**. MECANISMO (verificado): a polaridade e' camada de BORDA, *"a PRIMEIRA coisa do decode"* (`decoder.py:150-161`), e roda `_separa_sufixo_polaridade(line1[6:])` — no `.8M` esse `line1[6:]` e' `M<meta>`, e o fim do meta e' o **NOME DA ULTIMA COLUNA** (forma `min_header` omite o size da ultima, `multi/core.py:413-414`). A polaridade nao conhece a gramatica do `.8M`: ve `Mobs.`, separa `('Mobs','.')`, e o parser recebe coluna chamada `obs`. ALCANCE MEDIDO (64 nomes = `string.punctuation` x {1,2} repeticoes, 26 valores fixos): **`.8M` 48/64 = 75,0% RT falso** (24 perdem so' a chave, **24 corrompem chave E valores**); **`.8H` 38/64 = 59,4%**, mesmo mecanismo — o `.8H` **NAO e' controle**; **0 warnings em todo o sweep**. Escapam 16, os que terminam em `* , : = \ ^ | ~`. **CONTRA-PROVA que isola a causa**: a MESMA coluna com uma SEGUNDA ao lado da **64/64 RT ok** — com 2+ colunas o meta ganha `,`/`=`, o nome deixa de ser o fim da linha 1, e a polaridade nao o alcanca. **O defeito e' da COLUNA UNICA**. **POR QUE A SUITE NAO PEGOU**: o gatilho e' o MODO que vence o `min()`, porque o modo poe ou nao um prefixo antes do nome — n=3 sai `#TCF.8M!obs.` (modo raw, prefixo `!`) e o RT FECHA; n>=5 sai `#TCF.8Mobs.` (modo tcf, prefixo vazio) e QUEBRA. **Qualquer teste de RT com coluna pequena passa.** E' a 6a recorrencia da familia do `T-GRAFIA-CHECKLIST` (*"a frase no ADR nao impediu — o teste e' que impede"*), e a PRIMEIRA **entre camadas** em vez de dentro de uma. Nomes reais que caem: `obs.` `qtd.` `valor_r$` `medida(m)` `id#` — `.` final e' comum em export de planilha. **NAO CONSERTADO** (src/tcf intocado): exige aprovacao e decidir onde cortar — escapar o nome contra o alfabeto da polaridade no emissor, ou ensinar a polaridade a nao agir quando o disc e' `M`/`H`. Nao varrido: Unicode, `drop_names=True`, 3+ colunas |
...
> | ~~**`T-META-COLISAO-NOME-POSICIONAL`**~~ **SOLDADO 2026-08-16** (aprovacao do owner, item C2 da fila do M) | **FIX**: `_nomes_resolvidos(pairs)` em `multi/core.py` — FONTE UNICA core+view que resolve anonima->posicional E fail-louda em colisao; consumida por `_decode_multi_impl` e por `view.py::_parse`. **O ESCOPO CRESCEU NA EXECUCAO, e por um achado**: eu havia dito "1 linha no decode"; medindo antes de mexer, a **`view` falhava PIOR** — `_order` mantinha as 3 entradas (a contagem NAO denunciava) e `_col('0')` servia os bytes da 2a coluna nas DUAS chaves. Guard so' no decode deixaria a view servindo dado duplicado. Fonte unica e' tambem o precedente do proprio projeto (BUG-02: *"paridade view vs decode por CONSTRUCAO, nao por verificacao extra"*). **TESTES**: `TestMetaColisaoNomePosicional` em `test_f0_boundary_fixes.py` — 2 repros (decode+view) que falhavam ANTES (verificado vermelho) e 1 contra-prova de que encode legitimo nao dispara. Suite **1260 -> 1263 passed**, 3 skipped; gates byte-canonical verdes. **VERIFICACAO ADVERSARIAL PROPRIA**: (a) **12.000 encodes legitimos** (nomes arriscados `0`/`1`/`2`/`3`/`a`/`b`/`''`/`x` x 4 combinacoes de kwargs) -> **ZERO falso-positivo**; (b) 6 wires a mao, 4 com colisao e **2 de controle sem colisao** -> 6/6 corretos nos dois consumidores. O ENCODE nao emite a forma (chave de dict e' unica, `''` tem guard proprio, `drop_names` torna todas posicionais e distintas), entao o guard so' alcanca wire ESTRANGEIRO — e' o 4o cheque deduzido de graca da familia BUG-05. Lab que achou: `1450-ordem-de-colunas-no-M` bloco 3b (atualizado: agora documenta a correcao) | *(registro original)* decode de wire estrangeiro **PERDE COLUNA CALADA** quando nome explicito colide com posicional | reproduzido no lab `1450-ordem-de-colunas-no-M` (bloco 3b, fluxo invertido): wire a mao `#TCF.8M!3,!3=0,!fim` — coluna ANONIMA na posicao 0 (decoda `'0'`) + coluna NOMEADA `"0"` -> as duas viram a chave `'0'`, o dict SOBRESCREVE, **header declara 3 colunas e o decode devolve 2**; os valores da anonima somem sem warning. Os cheques do BUG-05 nao pegam (bytes e n_rows fecham). O ENCODE nao emite essa forma (chaves de dict sao unicas; `''` tem guard proprio em `multi/core.py:316-326`) — e' so' decode-de-wire-estrangeiro, mas a regua do proprio BUG-05 ("integridade deduzida de graca") cobre: **`len(result) == len(pares do header)` e' 1 linha de fail-loud** em `_decode_multi_impl`. Mexe em src, aguarda aprovacao. Nota-mae §4.4 |
> | **REGISTRO — AUDITORIA DO `.8M` NO CORPUS REAL** (2026-08-16, pedido do owner: *"focar ao maximo no 8M e ver se todas as otimizacoes e orientacoes de fluxo estao OK"*) | lab `2130-auditoria-do-M-no-corpus`, 23 tabelas / 186 colunas de `Z:/tcf-data/interim`, **0 falhas** | **O `.8M` ESTA SAUDAVEL, e agora verificado em dado REAL** (tudo antes era sintetico; o precedente do `T-DATA-ALVO-MENSAL` e' 95% sintetico -> 0,0% real): RT **23/23** · paridade `view`x`decode` **23/23** · as **6 invariantes de fronteira 23/23 cada, INCLUINDO decode PARALELO == serial** com `src/tcf` intocado (a orquestracao e' externa — o formato nao precisa de nada) · os 3 guards recem-soldados com **ZERO disparo espurio**, mesmo com **151 das 186 colunas reais** tendo nome com char NAO-alfanumerico (`education-num`, `capital-gain`, `marital-status`). **OS 4 CANDIDATOS TEM TODOS DOMINIO REAL** — dict 70 colunas · tcf 59 · split 37 · raw 20 — nenhum e' compute desperdicado, o que valida o `min()` do `_best_of` como esta'. **CORRECAO DE UMA EXTRAPOLACAO MINHA**: eu havia dito, do adult-census, que a soma dos wires flat batia o `.8M` em **+27,2%**, e usei isso pra dimensionar o Grupo A. **No corpus inteiro e' o CONTRARIO**: `.8M` 2.257.869 B contra 2.379.176 B da soma dos flats = **o `.8M` VENCE por 5,1%** (perde em 11 tabelas, +14.521 B; ganha em 12, -135.828 B), e o **adult-census e' EXATAMENTE onde ele mais perde** (+8.976 B = 62% de toda a perda do corpus). **Generalizei do PIOR CASO.** **O TETO REAL DA UNIAO (Grupo A) e' 2,3%, nao 27%**: medido coluna a coluna sobre os CORPOS (sem header), **77 de 186 colunas (41%)** teriam candidato melhor no flat e a uniao recuperaria **52.220 B**. **RECONCILIACAO com o `T-BAIXA-CARD-EM-TABELA`** (que registra 5x-12,8x): NAO ha contradicao — a razao POR COLUNA chega a **5,82x** (`adult-census:sex`, 2.014 B contra 346 B) mas **so' 10 de 186 colunas passam de 2x** e elas sao PEQUENAS em bytes absolutos; todas as piores sao **`bN` no flat contra `dict`/`tcf` no `.8M`**, o candidato ausente. **CONSEQUENCIA PARA A FILA**: o Grupo A segue certo como **unificacao de MANUTENCAO** (cada mecanismo soldado uma vez em vez de duas — o argumento original do `T-UM-CAMINHO-SO`), mas **o argumento de BYTES ficou fraco** e nao deve sustentar sozinho a decisao de abrir a gramatica do meta (B1). O maior retorno em bytes passa a ser o **`T-8H-UM-CANDIDATO-SO`**, que esta auditoria NAO tocou. **RESSALVAS**: amostra de 2000 linhas/tabela (0,3-0,4% nas maiores), janela CONTIGUA do meio (regua do lab `0530`); `NULL` vira string vazia; `tpch-sf001` e' prefixo do `sf01` e as duas entram, entao o TPC-H tem peso dobrado (16 de 23 tabelas); **nao e' teste de stress** |
> | **REGISTRO — o PISO do header do `.8M` e as 6 invariantes de fronteira** (2026-08-16, pedido do owner: *"tirar o maximo de explicitudes do header e fechar as questoes de limites de colunas pra preparar paralelismo"*) | lab `1530-piso-do-header-e-fronteira-paralela`, 0 falhas | **OS DOIS PEDIDOS ESTAO EM TENSAO E O PROJETO JA' RESOLVEU** — nao e' achado novo: o **O-FMT-19** (trocar byte-size por row-count) foi **REFUTADO** justamente porque *"custa TUDO: o lazy perde acesso O(1) por coluna, perde DECODE PARALELO (bytes deixam fatiar; linhas forcam scan) e group por slice"*; e o **O-FMT-11** ja' **FECHOU** o header como near-optimal (*"cada campo e' load-bearing pro decode independente"*), com o frontier real sendo o **O-FMT-14 (header derivavel)** — feature de CONTRATO, nao tweak de byte. **Os byte-sizes SAO o mecanismo de paralelismo: tirar explicitude tem PISO, e o piso e' a garantia que o owner quer preparar.** **PISO RE-VERIFICADO POS-WELDS** (o O-FMT-11 e' de 2026-07-05, anterior ao `:id`/ADR-0041, ao split `%` e ao FLOOR da nature): mesma formula `7 (magic) + Σ|size_hex| + (ncols−1) virgulas + marcadores + 1 LF`; 2 colunas anonimas dao **12 B de header** hoje contra os 13 B medidos la' (a diferenca e' size de 1 vs 2 hex — MESMO PISO). Break-even com dado REAL: N=5 7,4% · N=20 2,0% · N=100 0,5% · N=500 0,1% (mesma forma da curva original). **O `:id` custou +3 B de header e devolveu 4.450 B de corpo** (1.483x) — o header cresceu, e cresceu bem. **AS 6 INVARIANTES, DECLARADAS ANTES E TODAS OK**: I1 o plano de fatiamento sai SO' da linha 1 (82 B, sem tocar corpo) · I2 independencia (uma coluna nao le' byte de outra) · I3 ordem livre · **I4 PARALELISMO REAL (7 threads, resultado IDENTICO ao `decode()` publico)** · I5 so' a ultima depende de EOF · I6 plano completo (Σ sizes 20.450 + ultima 514 = corpo 20.964). **I4 e' a prova que faltava: o decode paralelo NAO precisa de nada novo no formato** — foi orquestracao EXTERNA sobre `_parse_meta` + os 4 decoders de modo, com `src/tcf` INTOCADO. **O CAMINHO BARATO**: `min_header=False` custa **+4 B (+0,019%)** e leva de 1 pra **0** as colunas que dependem de EOF — e' o perfil "pronto pra stream/paralelo", o kwarg JA' EXISTE, nao e' mudanca de formato e sim escolha. **ANATOMIA do que resta explicito** (soma bate exato com os 82 B): magic 7 (o `M` deduzivel = −1 B, marginal) · **sizes 18 = NAO removivel** · **nomes 37 = 45% do header, UNICO campo grande removivel** (`drop_names` leva 82→39 B, mas a ordem vira contrato — lab `1450` P2) · modos 4 · `:id` 4 (opt-in via `T-SPEC-SEM-CARIMBO`) · separadores 12. **VIEW (olhada leve, por decisao do owner fica pro fim)**: usa o MESMO `_parse_meta` (`view.py:117`), os sizes que reporta batem com o plano, e **materializa 0 B ao abrir** (`touched=[]`) — **"o header e' o unico coldstart" CONFIRMADO POR MEDICAO**. Nao investigado de proposito: as 8 formas que ele recusa, o bypass aritmetico, o `where` posicional. **CORRECAO DE METODO DENTRO DO LAB**: a 1a curva de break-even usava `v0..vN` (progressao que o seq-RLE esmaga — N=20 e N=100 davam o MESMO wire); refeita com dado real. Mesma classe do achado de amostragem do `0530`: dado sintetico regular esconde a variavel |
