...
|---|---|---|---|
| **`quantum`** | `x̂ ∈ {k·q}` | — | **mais forte que `abs`**: a norma monetária exige o valor *expressável* em centavos |
| **`abs`** | `\|x̂−x\| ≤ ε` | **soma** | é o invariante aditivo |
| **`rel`** | `\|x̂−x\|/\|x\| ≤ ε` | **produto** | GUM Eq. (12) |
| **`agg`** | `Σ x̂ = Σ x` no eixo declarado | — | restrição de **conjunto**; obriga **alocação** |
| **`mode`** | direção do desempate | — | duas normas com o mesmo `quantum` divergem no modo |
