...
input data
    |
    v
[CAMADA 0-pre]  Naturezas (opt-in via nature= / nature_per_col=)
    |  -- pre-transform por VALOR; fallback marker '_' se nao casa
    v
[CAMADA 0]      Pre-pass (filtros + heuristicas O(N))
    |  -- ColumnFeatures -> detect_cadence + detect_min_len
    v
[CAMADA 1]      OBAT (tokenizacao bidir LCP+LCS)
    |  -- processar() canonical OU processar_with_hint() shape-preserve
    v
[CAMADA 2]      HCC (M8.A composicional)
    |  -- detector greedy net>0 + emit operadores
    v
[CAMADA 2b]     HCC seq-RLE (M10, ADR-0011) + multi-delta (ADR-0016)
    |  -- post-process compacta runs near-identical
    v
[CAMADA 3]      Multi-column wrapper (header #TCF.6 M + meta line + concat)
    |
    v
output: texto TCF (`#TCF.6` single ou `#TCF.6 M` multi)
```

Cada CAMADA pode ser toggleavel via `PipelineConfig(pre_pass, obat_shape_preserve, hcc_seq_rle)`.
...
## 6. Dispatch, Multi-Column, PipelineConfig & Format Inventory
