...
| [BUG-VIEW-UMA-STRING-VAZIA](BUG-VIEW-UMA-STRING-VAZIA.md) | Uma única string vazia no corpo core faz `count()` devolver 0 e `select()` truncar a linha em silêncio. | **OPEN P1 2026-08-26** |
| [BUG-ENCODE-VAZIO-EM-COLUNA-TIPADA](BUG-ENCODE-VAZIO-EM-COLUNA-TIPADA.md) | coluna mista perde valor no `encode`/`decode`, e em `[1,'a']`/`[True,1]` emite wire que o próprio `decode` não lê. 17 variações medidas; contrato fechado 2026-08-26: sem tipo, mista vira texto; com tipo, não-membro vira nulo; preenchimento fora; modo estrito levanta. | **OPEN P1 2026-08-26** |
