...
| [T-QA-8](T-QA-8-material-comprobatorio.md) | **[dispositivo→exec]** Material comprobatório do #TCF.8/0.8.0: controle minúsculo (single c/s header, readers, README-propaganda) → sintéticos (curva de escala) → públicos (bench), com telemetria (SideOutputs + free Python/OS), TODOS os dicts (V2-B/implícito/natures + lab documentado) e verificação de paralelismo (byte-identidade RW + speedup + % serial). Regra CPF: medição DV-válido efêmero vs publicação anonimizada. §3 registra 10 bugs + 5 doc-drifts achados no planejamento (fix em F0 sob aprovação). Depois: otimização gated (F5) + pip com documento bem feito (F6). | **OPEN P1 2026-07-10** |
