...
| [T-REL-08-CLOSEOUT](T-REL-08-CLOSEOUT.md) | **[dispositivo→exec]** Fila única por ROI: R0 integridade do domínio aceito (BUG-14) → R1 F3/F4 → R2 F5 condicional+F6 → R3 C3. Corrupção/hardening ficam 0.8.1/pré-1.0; pesquisa fica .9. | **OPEN P1 · PRÓXIMO BUG-14/F3 2026-07-12** |
...
| [T-SPEC-STATUS-08](T-SPEC-STATUS-08.md) | **[dispositivo→registro]** Status dos specs (survey 3-agentes 2026-07-12) pra decidir o que fecha antes do teste em massa. Abordagem 1 (linguagem generalista/specs-induzidas) = mapeada, gated .9. Abordagem 2 (clássicos BR): ACHADO = gargalo é DADO, não máquina — `check_fn` é param LIVRE do construtor (5/7 clássicos cabem sem tocar core; CEP/PIS/renavam/título/CNH via construtor; telefone/RG/placa precisam máquina nova); mas só CNPJ (receita real) fecha o gate ≥15%/2-reais; CPF synthetic-teto. Decisão pendente A/B/C. | **CLOSED-DECIDED 2026-07-12** (owner: Opção A — só CPF/CNPJ no .8; clássicos → .9) |
| [T-SPEC-DEEPDIVE-08](T-SPEC-DEEPDIVE-08.md) | **[dispositivo→registro]** Investigação de fundo dos specs (redirect owner 2026-07-12, workflow 4-agentes). INSIGHT: a nature é pré-tx camada-0 FORÇADA (pré-empta o pipeline) em vez de COMPETIR no min() — por isso destrói estrutura inter-linha e piora em real ordenado (CNPJ +7339B) mas ajuda em embaralhado. CPF-modelo (10 eixos = gabarito); CNPJ estrutura medida (filial 97.6% const, base-8 delta, DV derivável → field-split ~50%); mapa valida Opção A (nenhum ganho de byte novo no .8); compilador JÁ é data-driven (un-weld = registry carregável pré-1.0). CRUX: nature-compete no 0.8 ou 0.8.1? | **OPEN P1 2026-07-12** |
