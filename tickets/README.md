...
| [T-CI-3-pyx-compiled-byte-gate](T-CI-3-pyx-compiled-byte-gate.md) | Caminho Cython COMPILADO além do dev: (1) gate byte-canonical do `.pyd` (suíte só valida pure-Python; ADR-0020 exige byte-equivalência .py↔.pyx mas nunca testa); (2) distribuição — wheels pré-compilados pra otimização chegar ao usuário, não só local (diretriz owner). Achado/registrado no P4. | **OPEN 2026-06-24** |
...
| [T-FMT-NAME-ESCAPING](T-FMT-NAME-ESCAPING.md) | Escape/quoting de nomes de coluna (`:`/`,`/`=`/`#`-inicial + `{}[]` da hierarquia) no meta — resolve o blocker do `:` sob .8-default via escaping (CSV-style), não rejeição. Interim backslash **WELDED** (M2, `58f7dee`); resta o estudo CSV-quoting/smart (deferido). | **OPEN-PARCIAL (interim welded) 2026-07-10** |
| [T-QA-8](T-QA-8-material-comprobatorio.md) | **[dispositivo→exec]** Material comprobatório do #TCF.8/0.8.0: controle minúsculo (single c/s header, readers, README-propaganda) → sintéticos (curva de escala) → públicos (bench), com telemetria (SideOutputs + free Python/OS), TODOS os dicts (V2-B/implícito/natures + lab documentado) e verificação de paralelismo (byte-identidade RW + speedup + % serial). Regra CPF: medição DV-válido efêmero vs publicação anonimizada. §3 registra 11 bugs + 5 doc-drifts (fix em F0 sob aprovação). **F0 lote 1 (BUG-01/02/07) FIXADO 2026-07-10** (red→green, 546 passed, byte-neutralidade 122/122 + gramática última-anônima). Depois: otimização gated (F5) + pip com documento bem feito (F6). | **OPEN P1 · F0 lote 1 done 2026-07-10** |
...
| [T-API-BOUNDARY-CONTRACTS](T-API-BOUNDARY-CONTRACTS.md) | Revisão profunda das fronteiras/semânticas da API **antes do 1.0** (diretriz owner: núcleo trabalha strings mas há tipos anterior/próximo, diffs e specs; lote 3 deu o ISOLAMENTO — código identifica os casos, comportamento re-decidível). Tabela caso-a-caso (vazio/containers/conversão/name/natures cruzados/simetria decode). | **OPEN P2 pre-1.0 2026-07-10** |
| [T-FMT-META-STRICT](T-FMT-META-STRICT.md) | Integridade do meta além do lote 3: princípio dedução-do-cânone (só aceitar o que o encoder emite; grátis) vs redundância (checksum → trilho tcfx/O-FMT-20). Welded: whitelist escape, size/fecho/n_rows, versão. Pendentes vinculados: nature-id estrito, view incremental, invariantes V2-B/split, fusões geométricas (checksum). | **OPEN P3 2026-07-10** |
...
| [T-REL-08-CLOSEOUT](T-REL-08-CLOSEOUT.md) | **[dispositivo→exec]** ORDEM de fechamento do 0.8 (materializa o levantamento 2026-07-10 — "a ordem de execução gera ticket"): Passo 1 higiene de status (5 tickets welded→closed rastreável) → Passo 2 caminho crítico (F1 runner → F2-F4 medição → F5 gated → F6 docs+wheel → C3 GO do owner) → Passo 3 fronteira registrada (0.8.1: BUG-12; .9: codec H/bN/CSV-quoting/V2-B B-C; pré-1.0: contratos). | **OPEN P1 2026-07-10** |
