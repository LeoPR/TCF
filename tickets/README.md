...
| [BUG-ENCODE-VAZIO-EM-COLUNA-TIPADA](BUG-ENCODE-VAZIO-EM-COLUNA-TIPADA.md) | coluna mista perde valor no `encode`/`decode`, e em `[1,'a']`/`[True,1]` emite wire que o próprio `decode` não lê. 17 variações medidas; a regra de fallback para texto resolve as três classes de dano. | **OPEN P1 2026-08-26** |
