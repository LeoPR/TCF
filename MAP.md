...
| **CNPJ alfanumérico — o weld** (`SPEC_CNPJ_ALFA`/`:cnpja` base 36; `SPEC_CNPJ`/`:cnpj` base 10 INTOCADO; `cnpj_spec_para` escolhe) | [`docs/adr/0042`](docs/adr/0042-cnpj-alfanumerico-dois-specs.md) + [`docs/how-to/use-natures.md`](docs/how-to/use-natures.md) — soldado 2026-08-21; byte-neutro provado por diferencial (0/13.046); a regra "tem letra → alfa" foi MEDIDA e reprovada |
| **CNPJ: UM só, compacto POR VALOR** (numérico=7 chars byte-idêntico ao legado dentro do `:cnpja`; decode discrimina pelo comprimento; chooser 51/51; minúscula = classe CONTRATO H-15-06, mediria −35,6%) | [`docs/adr/0043`](docs/adr/0043-cnpj-um-so-compacto-por-valor.md) + lab `2026-08-21-0130-cnpj-um-so-compacto/` — refina o 0042 pela direção owner (numérico se dilui: 2,1×10⁻⁷ do espaço novo) |
