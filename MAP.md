...
| **Float e suas variantes: CONSOLIDADO** (o que fecha, grafia × valor, o parâmetro de tolerância, o aviso, e a fila) | `docs/theory/tipos-e-naturezas/float-e-variantes-consolidado.md` (**fonte única do ciclo de 2026-08-14**) |
...
| **Lazy/queryable view** (descomprimir o minimo) | `src/tcf/view.py` (`from tcf import view`; A4) · **o que da' pra perguntar, com o custo medido de cada operacao**: `docs/how-to/consultar-sem-decodificar.md` · reference de API `docs/reference/lazy-view.md` · design 0.9 `experiments/lab/dirty/notas/2026-06/hquery01-decode-dag-indices-design.md` |
| **Presença, nulo e ausência: os três blocos** (por que o wire distingue 3 estados por célula e a tabela só tem 2) | `docs/theory/tres-blocos/INDEX.md`: quickuse das 5 perguntas + 6 notas. `V`/`N`/`A`, com união e complemento, cobrem as **8** uniões possíveis; `domain of definition`/`antidomain` com fonte; o `missing` é **falso cognato** (Mongo=A, pandas/polars/R=N). Os defaults vêm de precedente ou de teorema, e **nada está implementado**: o portão de desempenho depende de medição |
| Knobs de encode + view (reference) | `docs/reference/encode-knobs.md`, `docs/reference/lazy-view.md` |
| **Divulgacao**: material pra apresentar o projeto. Fonte de noticia datada na raiz, um subdiretorio por CANAL, e figuras por lingua; regras e limites no README da pasta. As figuras saem de `scripts/make_divulgacao_figuras.py` | `docs/divulgacao/` |
...
| **Spec orienta, não manda: a TRIAGEM `.8`/`.9`/`2.0`** (o doc de decisão da direção do owner) | `docs/theory/tipos-e-naturezas/spec-orienta-nao-manda-triagem.md`: nenhum conjunto FIXO de alvos cobre (folha NEGATIVA nos 3 alvos; eixo **dia-ÚTIL** recupera 99%); `.8` = 2 correções aprováveis; `.9` = guarda-chuva "orienta" (parse×eixos+gates); `2.0` = raiz (nós de proximidade, Patricia, segmentação no motor) |
...
| **As duas similaridades do núcleo** (doc de DECISÃO: a tese, a evidência, o espaço de escolha e as 4 perguntas em aberto) | `docs/theory/comparacao/duas-similaridades-igualdade-e-proximidade.md`: igualdade e proximidade **não competem no mesmo `min()`**; **o spec escolhe um domínio onde a aritmética sobrevive ao dedup** (reenquadra o papel da semântica); 3 encaixes com custo/risco (`T-SPLIT-SINGLE-COL` barato · `T-CANDIDATO-SEM-DEDUP` · Patricia `H-TH-02`); tudo `.9`, nada bloqueia o `.8` |
...
| **Registries de DIREÇÃO** (o que já foi pensado; consultar ANTES de abrir ticket) | [`roadmap-hipoteses.md`](experiments/lab/dirty/notas/2026-05/roadmap-hipoteses.md): hipóteses `H-*` · [`futuras-otimizacoes-formato.md`](experiments/lab/dirty/notas/2026-05/futuras-otimizacoes-formato.md): `O-FMT-*` |
| **`schema=`: spec na API (parametro unico)** (substituiu `nature=`/`nature_per_col=`; str=nome do registry, int=posicao; futuro objeto `Schema` = contrato) | [`docs/adr/0047`](docs/adr/0047-schema-parametro-unico-de-spec.md) + [`tickets/T-API-SCHEMA-PRESCRITIVO`](tickets/T-API-SCHEMA-PRESCRITIVO.md) + `tests/test_schema_param.py` |
