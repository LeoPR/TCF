...
| **RLE intra-valor e perda orientada a erro** (estudo + análise crítica, 2026-08-14) | `experiments/lab/dirty/notas/2026-08/2026-08-14-2010-rle-intra-valor-e-perda-estatistica.md` — **já discutíamos desde 2026-06-16** (H-INTRA-01/02/03 · O-FMT-17 · "C" da família RLE), mas **nunca houve lab**; esta é a primeira medição. O núcleo captura **zero** run intra-valor (o OBAT só compara extremidades); curva **1,000 B/char, sem amortização**. Teto no corpus **0,0031%**, e **contra-prova**: colapsar runs em `o_clerk` **custa 1.744 B**. A grafia inline esbarra no `*` (já é separador) e no escape de dígito → desbloqueio é **H-REF-03**. E o **`*0\|` já produz o "RLE fantasma" hoje, sem guarda** (`T-RLE-COUNT-ZERO`). Na parte de perda: **vocabulário de 4 eixos + `mode`**, e a medição de que **preservar um agregado pode degradar outro** |
